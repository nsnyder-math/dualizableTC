...
Which group actions do we care about?  The main group we care about is $SO(3)$ because that's the group that comes up in studying oriented field theories in the cobordism hypothesis.  But there are closely related groups which come up which we also want to address.  In all cases we only care about the 3-type of the group.

\begin{itemize}
\item Subgroups of $SO(3)$ (namely $SO(2)$)
\item 
\end{itemize}
...
\begin{itemize}
\item An assignment to every point $x$ a $3$-path $R_x: S_x \circ S_x \rightarrow \mathrm{Id}_x$.
