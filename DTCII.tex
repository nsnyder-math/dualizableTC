...
Needless to say, the 4-truncation of $B\Omega S^2 \simeq S^2$ is simply $S^2$, and this is obviously minimal. \NS{Do we want these statements in environments to refer to later? Or maybe just a theorem at the beginning of the section stating all of them?}
...
The 4-truncation of $BSO(2)$ is $\CP^2 = S^2 \cup_q e_4$, and this is certainly minimal.
...
\end{proposition}
\begin{proof}
That $\pi_4(\Sigma \RP^2) = \ZZ/4$ is \cite[Prop. 6.5(3)]{MR1955357} (using that $P_{n+2}(k)= \Sigma^n \RP^k$). \NScomm{Not sure how to extract the second part from what's written in Wu. I added the pdf of Wu's book to Git in a papers folder. See page 97, the definition of $Y_2$ is on page 93-94.}
\end{proof}
...
\end{proposition}
\begin{proof}
The homotopy groups of $S^3$ determine the homotopy groups of $SO(3)$ which determine the homotopy groups of $BSO(3)$ as $0$, $\ZZ/2$, $0$, $\ZZ$, $\ZZ/2$ for $\pi_1$ through $\pi_5$.  Using the slightly surprising fact that the composite $BSO(3) \ra BSO \xra{p_1} K(\ZZ,4)$ \NS{Add reference to appendix?} induces multiplication by 4 on $\pi_4$, this in turn determines the homotopy groups of $BOrp(3)$ as $0$, $\ZZ/2$, $\ZZ/4$, $0$, $\ZZ/2$.  
...
Composition of $1$-paths is read from bottom-to-top, and composition of $2$-paths is read from left-to-right. \NS{Wrote this sentence, but I'm not sure it literally makes sense.}
...
\NS{Might want to switch the order of the last two steps, because then the start of this also serves as the proof that $4=0$ in $\pi_2(\Sigma \mathbb{RP}^2)$}
...
\node (step2) at (4.5,-3) {$S\cdot I$};
\node (step3) at (9,-3) {$I \cdot S$};

\node (trivialstep2) at (4.5,-4) {$S$};
\node (trivialstep3) at (9,-4) {$S$};

\draw [->] (start) --node[above]{$\coev$} (top1);
\draw [->] (top1) --node[above]{$\id$} (top2);
\draw [->] (top2) --node[above]{$\beta_{S,S^{-1}}$} (top3);
\draw[->] (top3) --node[above]{$\ev$} (end);

\draw[->] (top1) --node[below, left]{$\piv^{-1} \cdot \id$} (step1);
\draw[->] (step1) --node[below, right]{$\piv \cdot \id$} (top2);

\draw[->] (top1) .. controls (3,-2) .. node[below, left]{} (step2);
\draw[->] (step2) .. controls (6,-2)  ..node[below, right]{} (top2);

\draw[->] (step2) -- node[above]{$\beta_{S,I}$}(step3);
\draw[->] (step3) --node[right]{$\piv^{-1} \cdot \id$} (top3);
\draw[->] (step3) .. controls (11,-2) ..node[right]{$\id \cdot \piv$} (end);

\draw[->] (step2) -- node[right]{$\mathrm{unit}$}(trivialstep2);
\draw[->] (step3) -- node[right]{$\mathrm{unit}$}(trivialstep3);
\draw[->] (trivialstep2) -- node[above]{$\id$}(trivialstep3);

\draw[->] (start) .. controls (1,-3) .. node[left]{$\piv$} (trivialstep2);
\draw[->] (trivialstep3) .. controls (12,-3) .. node[right]{$\piv^{-1}$} (end);

\draw[->] (start) .. controls (0,-7) and (16,-7) .. node[below]{$\id$} (end);

\node at (4.5,-.75) {$\Downarrow$};
\node at (4.5,-2.25) {$\Downarrow$};
\node at (7.5,-1.5) {$\Searrow$};
\node at (8.1,-1.3) {$\beta_{S,\piv^{-1}}$};
\node at (10,-1) {$\Searrow$};
\end{tikzpicture}
