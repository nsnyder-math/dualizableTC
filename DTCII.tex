...
\begin{proposition}
The homotopy classes of $\Omega S^2$ actions on a 3-type $X$ are classified by the elements of $\pi_1(\Aut(X))$.
\end{proposition}

\begin{proof}
By definition a homotopy action of $\Omega S^2$ on a 3-type $X$ is a map $B \Omega S^2 \ra B\Aut(X)$.  Up to equivalence, that is an element of $\pi_2(B\Aut(X)) = \pi_1(\Aut(X))$.
\end{proof}
...
\end{equation}
Here $I$ now denotes the trivial double loop at the identity automorphism.  This construction provides a map $\pi_2(\Aut(X)) \xra{\eta} \pi_3(\Aut(X))$, which may also be called the figure-8 construction.

\begin{proposition} \label{prop-quadaction}
The homotopy classes of $Quad$ actions on a 3-type $X$ are classified by the elements $[S] \in \pi_1(\Aut(X))$ such that $\eta(q([S])) = 0 \in \pi_3(\Aut(X))$, that is such that the double figure-8 construction on $[S]$ vanishes.
\end{proposition}
\begin{proof}
By Proposition~\ref{prop-quadtrunc}, the attaching map of the 5-cell of $BQuad$ is $\eta q: S^4 \ra S^2$, where $\eta : S^4 \ra S^3$ is nontrivial and $q: S^3 \ra S^2$ is a generator.  It suffices therefore to know, first, that the above construction $\pi_1(\Aut(X)) \xra{q} \pi_2(\Aut(X))$ is given homotopy-theoretically by $\pi_1(\Aut(X)) = \pi_2(B\Aut(X)) \xra{q^\ast} \pi_3(B\Aut(X)) = \pi_2(\Aut(X))$, where here the map $q^\ast$ refers to pullback along the map $q: S^3 \ra S^2$, and, second, that the above construction $\pi_2(\Aut(X)) \xra{\eta} \pi_3(\Aut(X))$ is given by the pullback $\pi_2(\Aut(X)) = \pi_3(B\Aut(X)) \xra{\eta^\ast} \pi_4(B\Aut(X)) = \pi_3(\Aut(X))$ along $\eta: S^4 \ra S^3$.

Turning the condition around, we would like to know that the composite $\pi_2(B\Aut(X)) = \pi_1(\Aut(X)) \xra{q} \pi_2(\Aut(X)) = \pi_3(B\Aut(X))$ is pullback along $q: S^3 \ra S^2$.  This fact is true when any space $Y$ and its loop space $\Omega Y$ are substituted for $B\Aut(X)$ and $\Aut(X)$ respectively.  As it is a statement about a map from $\pi_2(Y)$, it suffices to check in the universal case $Y = S^2$ and for the element $(S^2 \xra{\id} S^2) \in \pi_2(S^2)$.  In that case, the construction~\eqref{eq-q} is the Pontryagin-Thom image of the following normally framed embedding of $S^1$ into $\RR^3$: take the normally framed figure-8 immersion of $S^1$ into $\RR^2$, compose with the inclusion $\RR^2 \ra \RR^3$ equipped with the trivial normal framing, and deform the composite to an embedding.  The resulting normally framed manifold does indeed represent a generator of $\pi_3(S^2)$.

For the second condition, we replace $B\Aut(X)$ in the composite $\pi_3(B\Aut(X)) = \pi_2(\Aut(X)) \xra{\eta} \pi_3(\Aut(X)) = \pi_4(B\Aut(X))$ by the universal space $S^3$ and check the condition on the element $(S^3 \xra{\id} S^3) \in \pi_3(S^3)$.  The construction~\eqref{eq-eta} produces the Pontryagin-Thom image of the following normally framed embedding of $S^1$ into $\RR^4$: take the previously described normally framed embedding of $S^1$ into $\RR^3$ and include it into $\RR^4$, adding a trivial normal framing direction.  This normal framed manifold represents a generator of $\pi_4(S^3)$ as required.
\end{proof}

%Said in more homotopy theoretic terms, $S$ is a map $S^2 \ra B^2 \Omega \Aut(X)$, and $q(S)$ is the pullback of that map along a (positive) generating map $q: S^3 \ra S^2$.

%Said in more homotopy theoretic terms, $A$ is a map $S^3 \ra B^3 \Omega^2 \Aut(X)$ and $\eta(A)$ is the pullback of that map along the nontrivial map $\eta: S^4 \ra S^3$.
...
\begin{prop}
The homotopy classes of $SO(2)$ actions on a 3-type $X$ are the homotopy classes of pairs $(S,E)$, where $S: S^1 \ra \Aut(X)$ is a map and $E$ is a null homotopy of $q(S): S^2 \ra \Aut(X)$, the figure-8 construction applied to $S$.
\end{prop}
...
\begin{proof}
As mentioned, the 4-truncation of $BSO(2)$ is $S^2 \cup_q e_4$.  The action map on the 2-cell, $S^2 \ra B\Aut(X)$, is provided by $S \in \Omega \Aut(X) \simeq \Omega^2 B\Aut(X)$.  An extension of this map over the 4-cell is a null homotopy of the composite $S^3 \xra{q} S^2 \xra{S} B\Aut(X)$, that is a path in $\Omega^3 B\Aut(X)$ from $q^\ast S$ to the identity.  The proof of Proposition~\ref{prop-quadaction} shows that such a null homotopy is provided by a null homotopy of $q(S) \in \Omega^2 \Aut(X) \simeq \Omega^3 B\Aut(X)$.
\end{proof}
