...
(0,.25) to [out=90,in=90] (.75,0);
\draw[linestyle,looseness=1]
(.75,0) to [out=-90,in=-90] (0,-.25);
\draw[linestyle,looseness=1]
(0,-.25) to [out=90,in=-90] (.375,0);
\draw[linestyle,looseness=1]
(.375,0) to [out=90,in=-90] (0,.25);
\end{tikzpicture}}
\end{equation}
represents the unique nontrivial 2-torsion class in $\pi_3(\Sigma \RP^2) = \ZZ/4$.
\end{lemma}
\nid We refer to the construction defined in the lemma as ``the Radford bean".  Note well that in this construction the left and right facing cups are applications of the nullhomotopy $R$ of $2S$, not applications of a cancellation between $S$ and $S^{-1}$.
\begin{proof}
...
\end{tikzpicture}}
\end{equation}
That homotopy shows that the bean is homotopic to the composite $S^3 \xra{2q} S^2 \xra{\inc} \Sigma \RP^2$.  The lemma follows, because $\pi_3(S^2) \xra{\inc_*} \pi_3(\Sigma \RP^2)$ is surjective.
\end{proof}
\begin{remark}
...
\begin{itemize}
\item Lemma that either $p=q$ or that $p=-q$, ideally with geometric proof.  If the former, say that $R$ satisfies the snake, if the latter, note that $R$ need not satisfy the snake.
\item If $R$ satisfies the snake, then direct diamond geometric proof that $4p=0$.  If not, figure out a no-snake proof.
\item Proposition that the map $SO(3) \ra Aut(X)$ is S, R, triv of (6). [or could say, triv of q]  Remark: SO(3) action is just that map such that eta q / 2 vanishes, ie no new data in this case.
\end{itemize}
}

%\CDcomm{The following lemma might be absolutely completely false.  What is true is that either $p=q$ or $p=-q$, but which one *is* a well defined fact, not just a sign issue, and it corresponds to the question of whether the universal $R$ satisfies the snake relation.}
%\begin{lemma} \label{lemma-pq}
%The following triangle commutes up to homotopy, where the left map is restriction:
%\begin{equation} \nn
%\xymatrix@R=15pt@C=10pt{
%& \Omega \Aut(X) \ar[rd]^-{q}& \\
%\Map(\RP^2,\Aut(X)) \ar[ur] \ar[rr]_-{p} && \Omega^2 \Aut(X)
%}
%\end{equation}
%\end{lemma}
%\begin{proof}
%That the triangle commutes up to sign follows from the fact that both the maps $\pi_3(S^2) \xra{\text{inc}} \pi_3(\Sigma \RP^2)$ and $\pi_2(\RP^2) \xra{\Sigma} \pi_3(\Sigma \RP^2)$ are surjective.  A more geometric proof is given by the following homotopy of decorated normally framed manifolds:
%\begin{equation} \nn
%\end{equation}
%\end{proof}
