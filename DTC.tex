...
The Baer sum equips the hom sets in any category with direct sums with a commutative addition. \CD{Baer sum usually refers to a sum on Ext, so this sentence might need explanation (or omission since the sum is clear).}
...
\end{align*}
where in each case $U$ denotes the forgetful functor. Both forgetful functors are faithful and $U: \Cat_R^\textrm{IC} \to \Cat_R$ is full. \CD{We should say what full and faithful mean for functors of 2-categories.}
...
\end{definition}
\CD{We should say what a direct sum of linear categories is.}
...
% There is an abelian completion too?

%This section should be called meditations on the Deligne tensor product. 
}
...
\begin{proposition}
	...
	\CD{What does the parenthetical clause mean?}
	\begin{enumerate}
	...
	\begin{enumerate}
		...
		\CD{Does item (b) only use preabelian (really just cokernels), or did you need abelian?}
...
\CD{The category of endofunctors of M is abelian? What did that use?  If not, need to say "right split".}
\CSP{The Category of endofunctors is of the same kind as M, hence it is an IC additive category. In particular the notion of short exact sequence makes sense, as does split short exact sequence.}
...
\CD{Remind me why you can't have an idempotent on a sum of primitive tensors?}	
...
Now let $Y \to Z$ be a surjective map in $M$, and let $X \to Z$ be any morphism. Consider the image in $\oplus_I \Vect$ under $i$. The morphism $i(Y) \to i(Z)$ remains surjective because $i$ is right exact. Since every object in $\oplus_I \Vect$ is projective, there exists a lift as in the following diagram,
\begin{center}
\begin{tikzpicture}
	%\node (LT) at (0, 1.5) {$$};
	\node (LB) at (0, 0) {$i(X)$};
	...
	\draw [->] (LB) -- node [above left] {$\exists$} (RT);
...
\end{tikzpicture}.
\end{center}
Thus in $M$ we obtain the following diagram, which, using the functorial section $s_x: X \to ri(X)$, yields the desired section. 
\begin{center}
\begin{tikzpicture}
	%\node (LT) at (0, 1.5) {$$};
	\node (LB) at (0, 0) {$ri(X)$};
	\node (RT) at (1.5, 1) {$ri(Y)$};
	\node (RB) at (1.5, 0) {$ri(Z)$};
	\node (LBX) at (0, -1) {$X$};
	\node (RTX) at (3, 1) {$Y$};
	\node (RBX) at (3, -1) {$Z$};
	%\draw [->] (LT) -- node [left] {$$} (LB);
	...
	\draw [->] (LB) -- node [below] {$$} (RB);
	\draw [->] (LBX) -- (RBX);
	\draw [->] (LB) -- node [right] {$p_X$} (LBX);
	\draw [->] (RB) -- node [above right] {$p_Z$} (RBX);
	\draw [->] (RT) -- node [above] {$p_Y$} (RTX);
	\draw [->] (RTX) -- (RBX);
	\draw [->] (LBX) to [out = 120, in = -120] node [left] {$s_X$} (LB);
	%\node at (0.5, 1) {$\ulcorner$};
	%\node at (1.5, 0.5) {$\lrcorner$};
\end{tikzpicture}.
\end{center}  	
