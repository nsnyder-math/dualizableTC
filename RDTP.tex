...
\CSP{I changed "dangerous" to "confining".}
...
A linear category is finite if and only if it is equivalent to the category $\Mod{A}{}$ of finite-dimensional modules over a finite-dimensional $k$-algebra $A$.
\end{proposition}
\begin{proof}  \NS{Should this be more parallel with the proof of Prop 2.23?} \CSP{Now it does parallel that proposition more closely.}
...
\begin{definition}
	A {\em linear monoidal category} is a monoidal category $\cC$ such that $\cC$ is a linear category and the functor $\otimes$ is bilinear.  A {\em tensor category} is an abelian rigid linear monoidal category.  A {\em finite tensor category} is a finite rigid linear monoidal category.   \NS{Added a definition of tensor category}
\end{definition}
