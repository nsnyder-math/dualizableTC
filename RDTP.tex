...
\begin{definition}
	Let $\cC$ and $\cD$ be tensor categories. A {\em $\cC$--$\cD$-bimodule category} is a bicategory with two objects $x$ and $y$ such that \CD{``finite tensor cats"? or are we going to define tens to mean finite.  if not need to carefully go through whole paper and add ``finite" at appropriate spots.}
	\begin{itemize}
...
\begin{example} \label{ex:ModulesAreModules}
	For any algebra object $A$ in a tensor category $\cC$, the category $\Mod{}{A}(\cC)$ of right $A$-modules in $\cC$ is a left $\cC$-module category.  Similarly, the category $\Mod{A}{}(\cC)$ of left $A$-modules in $\cC$ is a right $\cC$-module category. \CD{``finite tensor cat"}
\end{example}

\begin{example}
$\cC$ is naturally a $\cC$--$\cC$-bimodule category with the actions given by the tensor product and the natural transformations by the associator. \CD{Say what C is}
\end{example}
...
\begin{example}
	Let $\cM$ and $\cN$ be left $\cC$-module categories. The categories $\Fun_{\cC}(\cM, \cM)$ and $\Fun_{\cC}(\cN, \cN)$ of right exact $\cC$-module endofunctors are finite linear monoidal categories. \CD{Need more conditions ...}
...
\begin{tikzpicture}[yscale=0.8, xscale=0.8, smooth]
	...
	\begin{scope}[xshift=10.5cm, yshift=-1cm]
		...
		\node at (1,-.5) {$c \otimes (-)$};
	\end{scope}
\end{tikzpicture} \CD{why are some of the lines not vertical in the first picture?}
\end{center}
\end{proof}
...
\begin{lemma}{\cite[\S 2.11]{EGNO}} \label{lma:Enough_C-projs}
	If $\cC$ is a finite tensor category and $\cM$ is a finite module category in which the action is right exact in $\cC$, then there exists a $\cC$-projective $\cC$-generator. \CD{Has `finite module category' been used before?}
\end{lemma}   
\begin{proof}
...
\CD{The rest seems weirdly tacked on.  Feels like the paper ends here properly.  Not sure what to do about that.}
