...
\end{theorem}  \NS{Is this numbering ok?  Or should this get changed to Theorem 1?}
...
%	It is standard that, for $A$ a finite-dimensional $k$-algebra, the linear category $\Mod{A}{}$ satisfies the four conditions. \NS{I propose replacing this proof by a reference to Introduction to Representation Theory Chapter 9.}
	% The only property which is not immediate is that every module admits a {\em projective cover}, that is an epimorphism onto that module whose source is projective and whose kernel is superfluous. 
...
\begin{definition}
	A {\em linear monoidal category} will mean a monoidal category $\cC$ such that $\cC$ is a linear category and the functor $\otimes$ is multilinear. A {\em finite tensor category} is a finite rigid linear monoidal category.  \NS{I'm not sure if we want to say ``right exact" somewhere here.  Rigidity implies exactness of tensor product so it may not be necessary.} \CD{I think in any case we should be completely explicit --- eg say (after the def perhaps) that linear monoidal categories aren't assumed right exact (or are if we like), but finite tensor categories are necessarily right exact.}
\end{definition}
...
\end{example} \NS{I moved these examples earlier and added one new example} \CD{Ok.}
...
%If $\cM$ and $\cN$ are both $\cC$-$\cD$ bimodule categories, then a bimodule functor which restricts to the identity functor on $\cC$ and $\cD$ will be called a {\em $\cC$-$\cD$-bimodule functor}.
 The above description in terms of bicategories permits us to effortlessly import several 2-categorical theorems into the world of bimodule categories. For example we have the following characterization of equivalences of bimodule categories (cf. \cite{MR2664622}).  \NS{Here and elsewhere I corrected c.f. to cf.}
...
The following result is well-known \cite[Rmk 4]{MR1976459}, but we were unable to find a proof in the literature and it is needed in the proof of Lemma  \ref{lma:module-adjoint}.  \NS{Added reference}
...
\end{lemma}   \NS{I think this needs a right exactness of the module action assumption.}
\begin{proof}
...
\end{definition}
\nid The balanced tensor product is also known as the ``relative Deligne tensor product", because the (unbalanced) tensor product $\cM \boxtimes \cN$ of linear categories is often called the ``Deligne tensor product".  \NS{The punctuation and quotes looks weird to me, but I'm assuming it was intentional, and I don't really care, so I didn't change it.}
\CD{I changed rDtp to btp throughout.  There were numerous instances where rDtp had been abbreviated to Dtp; I tried to change those to btp or tp as appropriate, but it may be that there is a use or two of btp now that should really be tp --- hopefully we'll catch if so in final reading}
...
\begin{proof}
	...
	\end{equation*}
	is exact in each variable separately. It is also $\cC$-balanced via the associator of $\cC$.\NS{Clarified this sentence.} Thus $(2)$ implies both  $(1)$ and $(3)$. We will first prove (2), and then establish (4).  We wish to show that for any $\cD$ the category of right exact functors 
\begin{equation*}
...
\begin{proof}
	...
	\NS{I think we should say a few more words about why the theorem follows from monadicity of the adjunction.}
