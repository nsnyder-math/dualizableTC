...
\begin{itemize}
	\item[-] a {\em lax} functor of bicategories $F: \cN \to \cC$ is equipped with natural 2-morphisms \CD{Why notation $\cN$ and $\cC$?}
	\begin{equation*}
...
\begin{tikzpicture}[yscale=0.8, xscale=0.8, smooth]
	...
	\begin{scope}[xshift=10.5cm, yshift=-1cm]
		...
		\node at (1,-.5) {$c \otimes (-)$};
	\end{scope}
\end{tikzpicture} \CD{straighten lines in the first picture?}
\end{center}
\end{proof}
...
The goal of this subsection is to prove a generalization of the following result. \CD{Need to explain specifically why we need a generalization of the result.}
