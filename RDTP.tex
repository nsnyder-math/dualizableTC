...
Our construction also addresses one technical issue.  The tensor product of categories comes in two slightly different flavors, due to Deligne~\cite{MR1106898} and Kelly~\cite{MR651714, MR648793}, depending on whether one considers abelian categories or finitely cocomplete categories.  In the non-balanced setting these tensor products agree when they both exists, but the Kelly tensor product is easier to work with and exists more often \cite{1212.1545}.  We show that under the assumptions of the main theorem, $\Mod{A}{B}(\cC)$ realizes both the balanced Deligne tensor product and the balanced Kelly tensor product, and so this technical issue does not cause any confusion in our setting.  This technical point is important because in the algebraic literature focuses on the Deligne tensor product, but from a higher categorical perspective the Kelly tensor product is easier to work with (see \cite[Rmk~3.21]{1501.04652}).  Specifically, in order to match up Johnson--Freyd and Scheimbauer's \cite{1502.06526} construction of a monoidal $3$-category of tensor categories in the finitely cocomplete setting with the abelian setting used in \cite{DTC1}, it is essential that the two balanced tensor products agree.
...
\begin{theorem} \label{thm:DelignePrdtOverATCExists}
	...
	\begin{enumerate}
		...
		It agrees with the balanced Kelly tesnor product. 
...
The rows of this diagram are exact\footnote{When $\cD$ is only additive, not Abelian, we take this to mean they are cokernel sequences.} (since $F$ was assumed to be right exact in each variable) and hence this diagram defines a unique map $\overline{\delta}_X$, shown as the long dashed arrow. We may define the value of the extension $\overline{F}$ on the object ${}_AX_B$ as the cokernel of $\overline{\delta}_X$. We leave it to the reader to verify that this extension gives a well-defined right exact functor 
\begin{equation*}
	\overline{F}: \Mod{A}{B}(\cC) \to \cD,
\end{equation*} 
