...
\end{warning} \CD{It had said ``3-morphisms'' here ...}
...
($1 \Rightarrow 2$) Finally, suppose that $G$ is left exact. Fix an object $d \in \cD$ and define a class $S_d$ of objects of $(d \downarrow G)$ as follows. An object $(c, f:d \to G c)$ belongs to $S_d$ if and only if for any sub-object $c' \subseteq c$ and factorization $d \to G c' \to G c$ it follows that either $c' \cong 0$ or $c' \cong c$. \CD{Really need given inclusion $c' \subset c$ to be iso, right?} The class $S_d$ consists of, in the language of \cite[Ex. 3-J]{MR0166240}, those objects $(c,f)$ that are {\em generated} by $d$. If there is a finite set $I$ of isomorphism classes of objects of $S_d$ then a choice of representatives $\{(c_i, f_i)\}_{i \in I}$ forms a collection satisfying the solution-set condition.  \CD{(Should we say more here? As is, no mention of left exact or finite length.)}
...
\end{example} \CD{Something about this example feels out of place}
...
\begin{lemma} \cite[2.1.8]{MR1797619} \cite[Prop. 1.13.1]{EGNO}  \label{lma:RigidIsExact}
	Let $(\cC, \otimes)$ be a finite tensor category. The multilinear functor $\otimes: \cC \times \cC \to \cC$ is exact in both variables. 
\end{lemma}
...
\end{proof} \CD{omit the last sentence?}
...
\end{itemize} \CD{Notation $\cN$ and $\cC$?}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item[-] the hom categories $\Hom(x,x)$, $\Hom(y,y)$, and $\Hom(y,x)$ are respectively $\cD$, $\cC$, and empty. \CD{That should be $\cC$, $\cD$, empty, right?}
...
\begin{proof}
We do the lax case, the oplax case is similar.  Suppose that $f_{c,m}:  c \otimes F(m) \rightarrow F(c \otimes m)$ is a binatural transformation making $F$ into an lax module functor.  The inverse to this natural transformation is given explicitly by the mate of $f_{c^*,m}$ 
...
Let $\cC$ and $\cD$ be tensor categories. Let  $\cM$ and  $\cN$  be finite $\cC$--$\cD$-bimodule categories and let $F: \cM \to \cN$ be a $\cC$--$\cD$-bimodule functor.  If the underlying functor of $F$ has a right (respectively left) adjoint as a functor, then $F$ has a right (resp. left) adjoint $\cC$--$\cD$-bimodule functor. 
\end{lemma}
\begin{proof}
...
The binatural transformation $\psi_{x,n}: x \otimes G(n) \rightarrow G(x \otimes n)$ is given by the mate:
$$x \otimes G(n) \rightarrow G F(x \otimes G(n)) \rightarrow G(x \otimes FG(n)) \rightarrow G(x \otimes n)$$
where the first map is the unit of the adjunction, the second map is the binatural transformation coming from the module functor structure on $F$ and the third map is the counit.  
...
\begin{center}
\begin{tikzpicture}[yscale=0.6]
	\node (A) at (2,2) [minimum height=1cm,minimum width=2cm, draw] {$f^{-1}_{x,G(-)}$};
	\draw (0,0) -- (0,3) arc (180:0:0.75cm) |- (A.135);
	\draw (A.225) -- (1.5,0);
	\draw (A.45) -- (2.5,4);
	\draw (A.315) -- (2.5,1) arc (180:360:0.75cm) -- (4,4);
...
\begin{example}
Consider $\Vect$ as a module category over $\Vect[G]$.  Then $\IHom(1,1)$ is $k[G]$ with each $g$ put in grade $g$.
\end{example}
...
\begin{example}
$\Vect$ as a $\Vect[G]$-module category can be realized as the category of modules over the group algebra $k[G] $ thought of as an object of $\Vect[G]$ by putting $g$ in grade $g$.  
\end{example}
...
\begin{proof}[Proof of Thm.~\ref{thm:C-module-Embedding}]
	...
	\begin{align*}
		...
		& \cong \Hom_{\cM}( F(b), x)
	\end{align*}  \NS{The cong's were missing on the last two lines, so I added them}
...
\begin{proof}
	...
	\end{equation*}
	is exact in each variable separately. It is also $\cC$-balanced via the associator of $\cC$.\NS{I added what the balancing is.} Thus $(2)$ implies both  $(1)$ and $(3)$. We will first prove (2), and then establish (4).  We wish to show that for any $\cD$ the category of right exact functors 
\begin{equation*}
...
\NS{Looking at DTC1, we don't seem to refer to this paper for those issues.  I don't think anything new is needed, but let me know if you have particular things in mind.}
...
\begin{example}
$\Vect \otimes_{\Vect[G]} \Vect$ is the category of $k[G]$-mod-$k[G]$ bimodules in $\Vect[G]$. Any left $k[G]$ bimodule is free, so we can identify $k[G]$-mod with $\Vect$, and thus $k[G]$-mod-$k[G]$ can be identified with $\text{mod-}k[G] \cong \Rep(G)$.
\end{example}
...
In the case where $\cC = \Vect$, that is in the case of the ordinary Deligne tensor product, a stronger version of Thm~\ref{thm:DelignePrdtOverATCExists}(4) holds under the assumption that we are working over a perfect field.
...
\begin{example}
	The assumption that the field is perfect is essential. If $k$ is imperfect, let $L$ be a finite inseparable field extension. The category $\Vect_L$ of $L$-vector spaces, with the usual tensor product over $L$, is a semi-simple tensor category. However the Deligne tensor product
	\begin{equation*}
		\Vect_L \boxtimes_{\Vect_k} \Vect_L \simeq \Mod{L\otimes L}{}
	\end{equation*} 
...
\begin{proposition}[`Products of module categories are modules over the product']
	%\label{lem:}
