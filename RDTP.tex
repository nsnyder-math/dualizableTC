...
\begin{lemma}\label{lem:module-adjoint-main}
	Let $\cC$ and $\cD$ be linear monoidal categories. Let  $\cM$ and  $\cN$  be  $\cC$--$\cD$-bimodule categories, and let $F: \cM \to \cN$ be an oplax (respectively lax) $\cC$--$\cD$-bimodule functor. If the underlying functor of $F$ has a right (respectively left) adjoint as a functor, then this adjoint admits the structure of a lax (respectively oplax) $\cC$--$\cD$-bimodule functor. Moreover if $F$ is a strong $\cC$--$\cD$-bimodule functor and the unit and counit of the adjunction are isomorphisms, then the adjoint is also a strong $\cC$--$\cD$-bimodule functor. 	
\end{lemma}
...
Providing the structure of a $\cD$-module functor is similar, and these left and right module functor structures are compatible---that is they form a bimodule functor structure.

If the unit, counit, and transformations $f$ are invertible, then the above transformation will also be invertible, and hence $G$ will be strong. 
\end{proof}
...
\begin{proof}
	By Lemma~\ref{lem:laxisstrong}
\end{proof}

 We have the following characterization of equivalences of bimodule categories.
