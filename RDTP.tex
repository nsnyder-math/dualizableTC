...
\begin{example}
	...
	\end{equation*} 
	is equivalent to the category of $L \otimes L$-modules. This category is no longer semisimple (since $L$ is inseparable) and moreover the induced monoidal structure is the tensor product over $L \otimes L$, which is no longer exact in each variable. It follows (C.f. Lemma~\ref{lma:RigidIsExact}) that $\Vec_L \boxtimes_{\Vec_k} \Vec_L$ fails to be rigid. 
\end{example}
