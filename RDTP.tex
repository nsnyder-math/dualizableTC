...
\begin{definition}
	A {\em linear monoidal category} will mean a monoidal category $\cC$ such that $\cC$ is a linear category and the functor $\otimes$ is multilinear. A {\em finite tensor category} is a finite rigid linear monoidal category.  \NS{I'm not sure if we want to say ``right exact" somewhere here.  Rigidity implies exactness of tensor product so it may not be necessary.} \CD{I think in any case we should be completely explicit --- eg say (after the def perhaps) that linear monoidal categories aren't assumed right exact (or are if we like), but finite tensor categories are necessarily right exact.} \CSP{Actually I think this is fine as written. By default there is no exactness, and I don't see a reason to add it in? 
	The very next lemma spells out the exactness properties when we are rigid.}
\end{definition}
...
\NS{Should we cite Barr-Beck somewhere?}\CSP{I don't think so.}
...
\end{lemma}   \NS{I think this needs a right exactness of the module action assumption.} \CSP{I added this to the statement.}
\begin{proof}
