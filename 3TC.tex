...
\CSP{These conventions are not set out at the end of the intro.}
...
\begin{definition}
	If $C$ is an extensive category with a terminal object and pullbacks then an internal category $X$ is a simplicial object in $C$ such that $X_0$ is discrete and such that the diagrams induced by those in equation (S),
	\begin{center}
	\begin{tikzpicture}
...
\end{definition}
 ...
 then a category enriched in $C$ is merely an internal category $X$ in which the object $X_0$ is {\em discrete}, . 




\CSPcomm{---- the below just needs to be totally rewritten -----}
