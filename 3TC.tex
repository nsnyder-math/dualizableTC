...
\CSP{Should these two paragraphs be moved to a different part of the paper? In the intro perhaps?}
...
An equivalence of bicategories will mean the usual sort: a functor $F: \cA \to \cB$ such that there exists a functor $G: \cB \to \cA$ and transformations $GF \to id_{\cA}$ and $FG \to id_{\cB}$ which are invertible up to invertible modification. Occasionally $\cA$ and $\cB$ will be 2-categories and $F$ will be a strict functor of 2-categories which is an equivalence when viewed as a morphism of bicategories. In this case we will say that $F$ is a {\em weak equivalence} of 2-categories. 
