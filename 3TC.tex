...
The most promising and immediate applications of these higher categorical developments appear to lie in the study of 3-dimensional  theories, and the current work provides an essential tool for future study of these theories. 
...
Extended topological field theories have received renewed interest with the recent proof (due to Hopkins and Lurie \cite{??}) of the Baez-Dolan cobordisms hypothesis. This result concerns {\em fully local} extended topological field theories in which the dimension of the bordism category matches the categorical dimension. Such theories allow one to decompose manifolds along submanifolds of arbitrary codimension. 

\CSPcomm{The cobordism hypothesis uses $(\infty,n)$-categories, it says that the cobordism category equipped with tangential framed is free, etc etc}


  is a symmetric monoidal $3$-functor from the 3-dimensional bordism $3$-category to a target symmetric monoidal $3$-category, which deloops the 2-category  linear categories. In this paper we construct such a 3-category. The objects consist of linear tensor categories, the morphisms are linear bimodule categories, the 2-moprhisms are bimodule functors, and the 3-morphisms are bimodule natural transformations. 


The goals of the current work are modest, yet are essential for any future developments in the study of extended 3-dimensional  topological field theories. 
...
We begin in Sections \ref{sec-symmonGamma} and \ref{sec-Compdelta} with a some expository material about symmetric monoidal $n$-categories, which we hope will make it more accessible to readers coming from topology or algebra. 

Section \ref{sec-symmoninftyNcat} gives a precise account of symmetric monoidal $n$-fold Segal categories, and Section \ref{} discuss the matter of full-dualizbality.
...
\CSPcomm{Following this reasoning gives rise to the notion of weak $n$-category constructed by Simpson and Tamsamani. In what follows we will describe $(\infty,n)$-versions of this construction, where sets are replaced by (simplicial) spaces.  }

\begin{definition}[Sketch]
	A Segal $0$-category is a Kan simplicial set. 	A {\em Segal $n$-category} is a simplicial object in $(n-1)$-fold simplicial spaces, such that the following conditions are satisfied:
	\begin{enumerate}
		...
		\end{tikzpicture}
		\end{center}
		is a (weak) pullback square of Segal $(n-1)$-categories,
	\end{enumerate}
\end{definition}

\noindent In the next section we will make this definition more precise. We will also review a few of the existing model categories of $(\infty,n)$-categories which are based on the above ideas.  
...
In this section we will briefly review two theories of $(\infty,n)$-categories which play a role in the current work. These are the theories of {\em $n$-fold complete Segal spaces} (due to Barwick) and of {\em Segal $n$-categories} (due originally to Hirschowitz and Simpson). Both are constructed inductively, beginning with the standard Kan model structure on simplicial sets. Symmetric monoidal variants of these theories will also be explained. 
The former theory is important to this work because of its role in the proof of the cobordism hypothesis. The latter theory is important because it provides a model in which it is technically easier to construct the symmetric monoidal $(\infty,3)$-category of tensor categories, something we do in the sequel. We wish to be able to apply the results of \cite{MR2555928} directly to this symmetric monoidal $(\infty,3)$-category and for this we need to understand the relationship between these two model categories. We describe the comparison in this section, and in the following section we describe how to see the invariance of the dualizability filtration. These considerations will allow us, in subsequent work \CSPcomm{[cite ourselves]}, to identify the fully dualizable subcategory of this symmetric monoidal $(\infty,3)$-category along with its $O(3)$-action, while avoiding nearly all model category theoretic complications. 
...
\begin{theorem}[\cite{Barwick:thesis, 0905.0462, 1112.0040}]
	There is a left proper combinatorial simplicial Quillen model category structure on $n$-fold simplical spaces in which the fibrant objects are the $n$-fold complete Segal spaces. This model category arrises as a left Bousfield localization of the Reedy (=injective) model structure on $n$-fold simplical spaces.
\end{theorem}
...
\CSP{cite Pellesier?}
\begin{theorem}[\cite{1001.4071} \CSPcomm{cite others??}]
...
\section{Dualizability in Symmetric Monoidal $(\infty,n)$-Categories}

In this section we will describe how to access the space of fully-dualizble objects in a symmetric monoidal $(\infty,n)$-category, and describe an associated dualizability filtration. We begin with several well known examples of $(\infty,n)$-categories.


\begin{example}
	A set may be regarded as a constant simplicial space. Thus the nerve of an ordinary category may be regarded as a discrete simplcial space. A discrete simplicial space is a Segal 1-category if and only if it is the nerve of an ordinary category. In this case $NC_0 = \textrm{ob}\ C$, the spaces $\Hom_{NC}(a,b)$ are the sets $\Hom_C(a,b)$, and $\mathit{h}NC \cong C$ is isomorphic to the original category. 
	\end{example}
...
The above lemma is a special case of a well known  general result. The category of discrete Segal 2-categories forms a good model for the theory of bicategories. \CSPcomm{cite Leinster? Tamsamani? Lack?} 
...
For $n\geq 2$, given a Segal $n$-category we may extract a discrete Segal 2-category, which we may in turn regard as a bicategory $\mathit{h}_2X$, the {\em homotopy bicategory of $X$}. This may be obtained as follows. First, we regard $X$ as a simplicial object in Segal $(n-1)$-categories. Then we may apply the homotopy category functor levelwise to obtain a simplicial object in the category of categories. Finally applying the nerve yields a discrete 2-fold simplicial space, which is easily seen to be a Segal 2-category. This construction is clearly funcotrial and sends weak equivalences of Segal $n$-categories to equivalences of bicategories. Choosing two objects $a,b \in X_0$ allows us to extract a Segal $(n-1)$-category $\Hom_X(a,b)$. Repeating these two constructions we obtain an assortment of bicategories associated to any Segal $n$-category. 

\begin{definition}
	\CSPcomm{A bicategory is dualizable if...  a symmetric monoidal category is dualizbale if... }
\end{definition}
