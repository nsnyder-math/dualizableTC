...
\end{tikzpicture}
\end{center}
has a diagonal filler. If $M$ is a class of maps, then we will let $M^\boxslash$ denote the class of all maps which have the right lifting property with respect to all maps in $M$. Similarly ${}^\boxslash M$ denotes the class of all maps which have the left lifting property with respect to all maps in $M$.
...
\begin{definition}%\label{def:}
	A \emph{Quillen Model Category} is a complete and cocomplete category $\cM$ equipped with 
\end{definition}
...
\begin{theorem}
	...
	\CSPcomm{List its properties, e.g. every object fibrant and cofibrant (hence it is proper) etc}
\end{theorem}

\begin{proof}
	\CSPcomm{to be filled in.}
\end{proof}
