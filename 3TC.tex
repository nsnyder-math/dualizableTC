...
\begin{theorem}[ \cite{Barwick:thesis, 0905.0462, 1112.0040} ]
	There is a Quillen model category structure on $n$-fold simplical spaces in which the fibrant objects are the $n$-fold complete Segal spaces. This model category arrises as a left Bousfield localization of the Reedy/injective model structure on $n$-fold simplical spaces.
\end{theorem}



\CSP{needs attention}
\begin{definition}[ \cite{0905.0462, 1001.4071} cite others?? ]
	A $0$-fold simplicial space (i.e. a simplicial set) is a {\em $0$-fold Segal category} if it is a Kan complex. An $n$-fold simplicial space $X$, regarded as a simplicial object $X_\bullet$ in $(n-1)$-fold simplicial spaces is an {\em $n$-fold Segal category} if it is Reedy fibrant and the following conditions are satisfied
		\begin{enumerate}
			\item The $(n-1)$-fold simplicial space $X_0$ is constant and discrete.
			\item Each of the $(n-1)$-fold simplicial spaces $X_k$ is a  Segal $(n-1)$-category.
			\item For every $m, n$, the Segal square,
			...
			\end{tikzpicture}
			\end{center}
			induces an equivalence of $(n-1)$-fold Segal categories $X_{m+n} \to X_m \times_{X_0} X_n$.  
				\end{enumerate}
\end{definition}

In order to complete the above definition we will need to supply the notion of equivalence. 



\CSPcomm{generalize to both to $\cM$, Lurie's theorems, Simpson's theorems. Putting it all together. Summary of facts (like homotopy category etc.)}
