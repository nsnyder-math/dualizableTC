...
\color{black}
\item[Repl] We had originally intended to include what we needed about the balanced tensor product in our paper ``Dualizable Tensor Categories".  However, we discovered that the literature on the balanced tensor product was insufficient to allow us to concisely cite the results we needed from elsewhere, and a separate paper on the balanced tensor was essential for us to be able to do the work we wanted to in 3-dimensional field theory.  It would be inappropriate to combine the paper ``The balanced tensor product of module categories" and ``Dualizable tensor categories", as their primary focuses are very different, and will be of interest and use to somewhat different audiences.  (Furthermore, the length of the paper ``Dualizable tensor categories" is already too long for most journals, and adding more material too it would exacerbate the situation.)  We therefore have elected, of the referee's two options, to revise the present paper to clarify the point below.
...
\color{black}
\item[Repl] Added a reference to the book.  We agree that much core work is done in EGNO, but we were not able to find enough of the specific details we needed to condense this paper further than we have.
...
\color{black!50}
\item p. 3 middle, Definition 2.6 lacks in explaning the definition of bimodule functor.
\color{black}
...
\color{black}
\item[Repl] We have clarified which implications were handled where.
