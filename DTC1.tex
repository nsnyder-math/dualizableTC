...
\CD{Shall we use $\times$ instead of $\ast$ in those duality equations?}
\CSP{This is the whiskering of nat. transformations. I agree $*$ is not the best, but $\times$ is no good either. The `$\eta * id_{G}$' is a natural transformation with $G\circ F \circ F$ as source, so $\circ$ is a candidate, but that usually means vertical composition. }
\CSP{trying out $\circledcirc$}
\end{definition}
With this standard convention we have a canonical natural isomorphisms $\Hom_\cA(F(x), y) \cong \Hom_\cB(x, G(y))$. 
\begin{definition} [Convention B] \label{def:duals}
...
We impose one further convention (Convention (C)), which is that Convention (A) and Convention (B) should hold simultaneously in the above example. 
