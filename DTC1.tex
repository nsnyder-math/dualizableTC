...
\begin{remark}
Consider the interval with one endpoint incoming, the other endpoint outgoing, and both endpoints being the standard positively framed point.  Framings on this interval form a group under composition and can be canonically identified with $\pi_1(\mathbb{SO}_n)$ (with the positively framed point as the basepoint).  When $n=2$, framings on the interval can be canonically identified with $\mathbb{Z}$.  The right universal Serre automorphism corresponds to $+1$ in this group, while the left Serre automorphism corresponds to $-1$.  When we refer to just the Serre automorphism, we mean the right Serre automorphism.
\end{remark}
...
We now specialize to our case of interest, namely where the object in question is a finite tensor categroy (which is a dualizable object in the $(3,2)$-category $\TCrig$).
...
If $\cC$ is a finite tensor category, then the Serre bimodule $\bimod{\cC}{\cS}{\cC}$ for $\cC$ is Morita equivalent to the $\cC$--$\cC$ bimodule associated to the left double dual monoidal functor $\ldd{(-)}: \cC \ra \cC$.
\end{theorem}
