...
\begin{tikzpicture}[remember picture,overlay] 
	\node [yshift = -2cm, red!80!black,scale=3,text opacity=0.2] 
...
The proof that fusion categories are fully dualizable builds on the algebraic approach to finite tensor categories developed by Etingof, Gelaki, Nikshych, and Ostrik \cite{EGNO, MR2119143, MR2183279, MR2097289}.  This approach was originally developed to avoid the unnecssary assumptios, like sphericality, that arose in earlier topological approaches.  Nonetheless, many of the algebraic results turn out to be well motivated from the topological point of view.
...
\NScomm{Note that there is another option.  In order to intepret $x^*$ as an adjoint, we need to interpret $x$ as a morphism and $\otimes$ as composition.  As always there are competing conventions about whether $f \otimes g$ means $f(g(-))$ or $g(f(-)$.  We were implicitly using the former convention.  If you use the latter then Conventions (A) and (B) hold, and such that $x^*$ is a right dual as in EGNO and ENO.  We plan to change to these conventions in a later version of the paper.}
...
\begin{definition}
If $\cC$ is a fusion category we define its global dimension to be \NScomm{figure out what the best definition is for CSP's argument, and make sure that it makes sense when the unit is not simple}
\end{definition}
...
\begin{example}
Here is a picture of a $2$-framed manifold: \NS{Insert an example}
\end{example}
...
.
\NScomm{This section is under construction.  The current outline is more detailed, since when it was written we were planning on constructing non-compact theories in this paper}
...
Thus the fact that $\pi_1(\mathrm{SO}_3) \cong \mathbb{Z}/2$ has the following consequence in $\TCsep$.
...
\NS{We need to check conventions here for order of composition, I probably got them all wrong}
\begin{align*} 
...
\end{theorem}
\begin{proof}
\NScomm{CD and CSP are working on this.  The main content of the proof is topological, that is using an explicit descriptions of $\mathrm{SO}_3$ and $\mathrm{SO}_2$ you can show that the data needed to turn a $2$-framed theory into a $3$-framed theory is exactly a trivialization of the square of the Serre automorphism.}
\end{proof}
