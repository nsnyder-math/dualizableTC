...
\subsection{Linear Categories and Tensor Categories} \label{sec-tc-lincat}
	...
	Let $k$ be a fixed ground field. A {\em linear category} is an abelian category with a compatible enrichment over $\Vect_k$, the category of $k$-vector spaces. 
	\CSP{What is our notational difference for finite dimensional vector spaces versus all vector spaces.}
A {\em linear functor} is a right exact additive functor, which is also a functor of $\Vect_k$-enriched categories. 
...
\begin{remark}
	The potentially confusing notion $x^*$ for the {\em left} dual of $x$ is a product of the confluence of two equivalent notions of duality, one coming from work on monoidal categories and the other coming from the notion of adjoint functor.  
\end{remark}
