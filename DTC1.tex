...
\item For every object $a \in \cC$, there exists an object ${}^* a \in \cC$ that is a left dual for $a$ and there exists an object $a^* \in \cC$ that is a right dual for $a$.
\end{enumerate}
\end{definition}
...
\begin{proposition}
If $\cC$ is a fusion category, then there exist monoidal functors ${}^*(-) : \cC \ra \cC^{\op}$ and $(-)* : \cC \ra \cC^{\op}$ whose values on any object $a \in \cC$ are respectively a left dual object ${}^*(a)$ and a right dual object $(a)^*$ for $a$.
\end{proposition}
\begin{proof}
Define the functor ${}^*(-)$ on objects by picking for each object $a \in \cC$ a left dual object ${}^*a \in \cC$.  Also pick a unit map $u: {}^*a a \ra 1$ and a counit map $v: 1 \ra a {}^* a$ giving ${}^* a$ the structure of a left dual to $a$.  Define the functor ${}^*(-)$ on a morphism $f: a \ra b$ by ${}^*(f) := (u_b \cdot \id_{{}^*a}) (\id_{{}^*b} \cdot f \cdot \id_{{}^*a}) (\id_{{}^*b} \cdot v_a)$.  (This morphism ${}^*(f)$ is called the \emph{mate} of $F$.)  Next we want to see that this functor is monoidal.  Observe that the morphisms ${}^*b {}^*a a b \xra{u_a} {}^*b b \xra{u_b} 1$ and $1 \xra{v_a} a {}^*a \xra{v_b} a b {}^*b {}^*a$ show that $({}^*b {}^*a)$ is a left dual to $(a b)$.  There is therefore a uniquely determined isomorphism from $({}^*b {}^*a)$ to ${}^*(a b)$.  These isomorphisms provide the left dual functor with a monoidal structure.  The right dual functor is analogous.  % In principle, need to check naturality and hexagon for those isos.
\end{proof}
...
\begin{proposition}
Let $a$ be a dualizable object of a symmetric monoidal 3-category $\cA$.  The square $\cS_a^2 : a \ra a$ of the Serre automorphism $\cS_a$ of $a$ is Morita equivalent to the identity of $a$.
\end{proposition}

\begin{proof}
\CDcomm{It's true for the universal dualizable object.  Establish that above during discussion of 3-framed 1-manifolds}
\end{proof}
...
If $\cC$ is a fusion category, then the Serre bimodule $\bimod{\cC}{\cS}{\cC}$ for $\cC$ is Morita equivalent to the $\cC$-$\cC$ bimodule associated to the double dual monoidal functor $**: \cC \ra \cC$.
\end{theorem}
