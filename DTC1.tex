...
\subsection{The symmetric monoidal 2-category of linear categories} \label{sec-tc-threecat}
...
\subsubsection{Adjoints of bimodule functors} \label{sec-df-functors}
...
If $\cC$ is a rigid monoidal category, $\cM$ and $\cN$ are left-module categories over $\cC$, and $\cF: \cM \rightarrow \cN$ is functor of underlying categories which has a right adjoint $\cG$, then $\cG$ has a natural module functor structure.  Similalry a left adjoint to a module functor has a natural module functor structure.  These results also hold for right module categories and bimodule categories.
\end{lemma}
\begin{proof}
The binatural transformation $X \otimes G(n) \rightarrow G(x \otimes n)$ is given by
$$x \otimes \cG(n) \rightarrow \cG \cF(x \otimes \cG(n)) \rightarrow \cG(x \otimes \cF\cG(n)) \rightarrow \cG(x \otimes)$$
where the first map is the unit of the adjunction, the second map is the binatural transformation coming from the module functor structure on $\cF$ and the third map is the counit.  Rigidity of $\cC$ guarantees that this binatural transformation is a natural isomorphism.  The compatibility condition is an easy exercise with commuting diagrams.
\end{proof}

\begin{remark}
If $\cC$ is not rigid the above argument only shows that the right adjoint to a lax module functor has an oplax module functor structure, while the left adjoint of an oplax module functor has a lax module functor structure.  
\end{remark}
...
\end{proposition}
\begin{proof}
This follows from checking the conditions of the Adjoint Functor Theorem.
\end{proof}

\begin{proposition}
If $\cC$ is a finite tensor category and $\cM$ and $\cN$ are module categories over $\cC$ with $\cM$ exact, then any module category functor $\cF:\cM \rightarrow \cN$ is exact.
\end{proposition}

\begin{corollary}
Let $\cC$ and $\cD$ be rigid monoidal categories, let $\cB$ a finite tensor category, let $\cM$ and $\cN$ be $\cC$-$\cD$ bimodules which are also left $\cB$-module categories where $\cM$ is exact as a $\cB$-module category, and let $\cF: \cM \rightarrow \cN$ be a $\cC$-$\cD$ bimodule functor which also has the structure of $\cB$-module functor.  Then $\cF$ has both a left and right adjoint as a $\cC$-$\cD$ bimodule functor.
\end{corollary}
\begin{proof}
Exactness of $\cM$ as a $\cB$ module gurantees that the underlying functor of $\cF$ is exact and thus has both left and right adjoints.  Then Lemma \ref{module-adjoint} shows that it has left and right adjoints as a bimodule functor.
\end{proof}

\begin{remark}
In applications of the above corollary $\cB$ will typically be $\mathrm{Vec}$, $\cC$, $\cD^{mp}$, or $\cC \otimes \cD^{mp}$.
\end{remark}

\begin{corollary}
If $\cC$ and $\cD$ are fusion categories, $\cM$ and $\cN$ are semisimple linear $\cC$-$\cD$ bimodule categories, and $\cF: \cM \rightarrow \cN$ is a module category functor, then $\cF$ has both a left and a right adjoint as a module functor.
\end{corollary}
\begin{proof}
Apply the above corollary with $\cB = \mathrm{Vec}$.
\end{proof}

\subsubsection{Adjoints of bimodules}  \label{sec-df-modules}

In this section we give sufficient conditions for a $\cC$--$\cD$ bimodule category to have left and right adjoints.
