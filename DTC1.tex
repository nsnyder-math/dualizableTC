...
In this section we give sufficient conditions for a $\cC$--$\cD$ bimodule category $\cM$ to have left and right adjoints.

\begin{definition}
Let $\cC, \cD \in \TCrig$, and let $\cM \in \Hom_\TCrig(\cC, \cD)$.  Define the left opposite module category $\cM^{R} \in  \Hom_\TCrig(\cD, \cC)$ to be $\cM^{op}$ as a category with the action $d \cdot m \cdot c$ given by $c^* \otimes m \otimes {}^*d$.   Define the right opposite module category $\cM^{L} \in  \Hom_\TCrig(\cD, \cC)$ to be $\cM^{op}$ as a category with the action $d \cdot m \cdot c$ given by ${}^*c \otimes m \otimes d^*$.
\end{definition}

\begin{proposition}
The bimodule functor ${}_\cD \cM^L \dtimes_\cC \cN_\cD \rightarrow {}_\cD \Fun_\cC(\cM,\cN)_\cD$ given by $x \otimes y \mapsto \IHom(x,y)$ is an isomorphism.
\end{proposition}
