...
An $n+1$-dimensional topological field theory, according to Atiyah and Segel, is a functor from the category of $n+1$-dimensional bordisms to the category of vector spaces.  That is, it assigns to any closed $n$-manifold a vector space, and to any bordism it assigns a linear function between the corresponding vector spaces.  In particular, to a closed $n+1$-manifold a TFT assigns an endomorphism of the trivial vector space, that is a scalar.  Hence, TFTs can be thought of as topological invariants which can be computed by chopping the manifold up into smaller pieces.   A fully local (or fully extended) TFT allows one to cut the manfiold not just along codimension one boundaries, but allows boundaries with corners.  To state this rigorously 
