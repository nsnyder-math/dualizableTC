...
%\begin{abstract}
%The Turaev-Viro construction assigns to a spherical fusion category a 3-dimensional topological field theory.  We generalize this construction in three ways. First we construct a local, that is fully extended, TFT.  Second, we relax the assumption of sphericality on the fusion category: an arbitrary fusion category gives a $3$-framed TFT.  Finally, we show that finite tensor categories give a local $3$-framed $2$-dimensional field theories.  This correspondence between finite tensor categories and TFTs allows us to translate algebraic statements and arguments into topology and vice versa.  We give two main examples of this correspondence.  First, we show that the fact that $\pi_1(\mathrm{SO}_3) = \ZZ/2$ directly implies that the quadruple dual functor on a fusion category is trivial.  Second, we explain how the sphericality of a fusion category corresponds to the spin-independence of the associated TFT.
%\end{abstract}


	
\begin{abstract}
	The cobordism hypothesis predicts the fully-dualizable objects of any symmetric monoidal $n$-category $C$ admit the structure of a homotopy $O(n)$-action, and that various associated the homotopy fixed point spaces correspond to various sorts of local (that is fully extended) topological field theories with values in $C$. Spherical fusion categories have a well-known connection to oriented topological field theories via the Turaev-Viro construction, which assigns to each spherical fusion category a 3-dimensional topological field theory. 
	
		We strengthen this relationship in several ways. We show that the fusion categories are the fully-dualizable objects of a certain symmetric monoidal 3-category, and explicitly identify a substantial portion of the predicted $O(3)$-action. This sets up a correspondence which allows us to translate algebraic statements and arguments into topology and vice versa. We show that the fact that $\pi_1(\mathrm{SO}_3) = \ZZ/2$ directly implies that the quadruple dual functor on a fusion category is trivial.  
Moreover our results generalize the Turaev-Viro construction in three ways. First we construct a local, that is fully extended, TFT.  Second, we relax the assumption of sphericality on the fusion category: an arbitrary fusion category gives a $3$-framed TFT.  Finally, we show that finite tensor categories give local $3$-framed $2$-dimensional field theories. We also explain how the sphericality of a fusion category corresponds to the spin-independence of the associated TFT.
\end{abstract}	
...
\begin{tikzpicture}[remember picture,overlay] 
	\node [yshift = -1cm, red!80!black,scale=3,text opacity=0.2] 
...
In addition to the local TFT story, there are two other narratives that this work fits into.  The early history of fusion categories was heavily topological.  However, the topological approach seemed to require extra assumptions (like sphericality for fusion categories or ribbon for braided tensor categories).  In part to avoid these unnecessary assumptions, a more purely algebraic approach to the theory of fusion categories was developed by Etingof, Nikshych, Ostrik, and others (e.g. \cite{EGNO, MR2183279,0906.0620,MR2097289}).  \CD{If there are any important ``others" they should be named, not left unnamed.} One of the main points of this paper is that this algebraic approach can be brought back in touch with topology.  In fact, many of the algebraic results turn out to be well motivated from the topological point of view.   \CD{The first half of this paragraph seems to be repeating things that have already been discussed above.} The second narrative is that this paper can be thought of as a category theoretic approach to the theory of fusion categories.  The main philosophy of category theory is that instead of studying something by looking inside them at their elements, you can instead study objects by looking outside them at their relationship to other objects.  Thus the category theoretic perspective on fusion categories avoids looking inside the category at the object and morphisms, and instead concentrate on the bimodules between fusion categories.  In this work we fully embrace this perspective, and only look inside the categories to establish a few key results, while the main results are proved by looking at bimodule categories, functors, and natural transformations.  \CDcomm{I think this paragraph is too philosophical (and I love philosophizing ...), given our gravitas at present.  I think it can be deleted from the introduction.  The first half is already discussed above, and we can put the idea in the second half as a remark somewhere in the paper.  Perhaps at a moment when we break rank and go inside the category we can remark that the reader will notice we have gone to great lengths to take an external viewpoint, but yadda yadda.}
\CD{If this paragraph is deleted, the four references should be sure to move elsewhere not get removed.}
...
\begin{corollary}
For any separable fusion category there is a local topological quantum field theory whose value on a point is that fusion category.  \CDcomm{I would remove this next sentence: Furthermore, this TFT is unique up to equivalence, and two fusion categories give the same field theory if and only if they are Morita equivalent.}
\end{corollary}
...
\begin{theorem}
Finite monoidal categories whose tensor product is biexact, are twice dualizable (that is, fully dualizable in the (3,2) truncation of the $3$-category of finite monoidal categories). \CD{Gaaa, this theorem should be rewritten.  Too technical and unclear for the introduction.  Maybe rephrase in terms of the kind of field theory that results.}
\end{theorem}

This shows that finite monoidal categories give $2$-framed local $2$-dimensional TFTs, which are categorified in the sense that they take values in a $(3,2)$-category instead of a discrete $2$-category.  \CDcomm{I think this paragraph also should be rewritten: ``(3,2)" should not be used in the introduction at all.  I think it's clearer to just say, it's categorified in the sense that it's a 2-dimensional theory where the invariant of a 2-manifold is a vector space, not a number, or something like that.  Or just not use the word 'categorified'.}
...
\begin{theorem}
The Serre automorphism of a finite monoidal category $\cC$ is the bimodule associated to the double dual functor $**: \cC \ra \cC$.  \NS{Left or right double dual?} \CD{It's the functor ${}^{**}(--)$ which we are still arguing about whether to call the left or right double dual.}
\end{theorem}
...
After we announced the results of this paper, other constructions of local versions of Turaev--Viro have been announced by Balsam--Kirilov and Kitaev--Kong.  \CDcomm{[I would delete or rewrite the rest of this paragraph.  I'm not sure it is complimentary, and almost anything we say might seem confrontational.  Also, it's not really our uniqueness results so much as Jacob's.  Though we may want to ``set ourselves apart" I think the work will have to stand on its own in that respect.]  Their work and our work are largely complimentary.  Their constructions depend on a choice of spherical or unitary structure and thus they cannot prove our main applications, on the other hand, their constructions explicitly recover the Turaev--Viro invariants for $3$-manifolds, which is more difficult to see from our construction.  Note that by our uniqueness results all of these local TFTs must be equivalent.}
...
We would like to thank Ben Balsam, Alexander Kirillov Jr., Scott Morrison, Dylan Thurston, and Kevin Walker for helpful conversations \NS{Chris's add any people you'd like to mention here}.   For the first six months of this project we collaborated using Google Wave, and we would like to thank the Google Wave team for their work, and Scott Morrison and Nigel Tao for arranging early access to Wave for the U.C. Berkeley math department. \CD{discuss}
...
Noah Snyder and Chrs Schommer--Pries were supported by NSF Postdoctoral Fellowships.  \NS{Other grants?} \CDcomm{These grants should move to the thanks section that is on the first page.}
...
\CSPcomm{[Maybe move this to section 5? It is totally new material, as far as I know.]} \CDcomm{I think logically it fits better in section 2 here.  Noah has already pointed out in the introduction that the material is new, and this can be rementioned by a sentence at the beginning of this subsection.  Said differently: if you think it will logically tie together better in section 5, that's fine, but I see no problem with having new material here in section 2.}
...
\CDcomm{Insert a few introductory sentences here, including a statement of the cobordism hypothesis.  That will motivate the need for a notation of n-framed manifolds, and for describing various structure groups.  Then we can forward reference to 4.3 the more detailed discussion of the cobordism hypothesis.}
