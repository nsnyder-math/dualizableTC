...
\subsection{Notation for $n$-framed manifolds} \NS{I think this subsection should come earlier in the section}
...
\[TM \oplus \RR^{n-k} \simeq TM \oplus \nu(\iota) \simeq  \RR^n.\] \NS{I deleted a $M \times$ in the last expression because I think it was a typo}
...
When drawing $n$-framed manifolds using the normally framed notation described above, we draw the immersed manifold and draw the normal framing, and leave the induced $n$-framing completely implicit.  When the immersion is codimension-1, the normal framing may be indicated simply by a unidirectional gray corona coming off the manifold.  Moreover, as in the codimension-1 case this corona can propogate without ambiguity, it may be indicated only at one point of the manifold.

\begin{example}\NS{I added some examples earlier on, because having a picture on the first page of the subsection will encourage people to read it.}
...
In drawing manifolds with boundary and corners, we also need to specify which parts of the boundary are incoming and outgoing.  Outgoing pieces of the boundary will be indicated by a small arrow pointing out of the bulk of the manifold; incoming pieces of the boundary will be undecorated---implicitly the arrows would point into the bulk.  \NS{I vote for decorating all boundaries.  It's clearer and shouldn't be substantially more work.}  When the immersion is codimension zero, the outgoing boundary arrows may be replaced by a gray corona, which serves to directly record the induced normal framing of those parts of the boundary.  At corners, a combination of these indications will be used.  For instance, an arrow together with a corona on a codimension 2 corner indicates respectively the first and second vectors of the induced framing on the corner.
...
\[\textrm{point, point with neg arrow, point with two +45 rotated arrows labelled 1,2, point with two +45 rotate arrows labelled 2,1}\]
\end{example}

\begin{example}
Here is a picture of a $1$-framed manifold:
\[\textrm{picture of horizontal line, corona on left}\]
...
\[\textrm{picture of curve in plane with boundary at +45 and -45, with two loops and upward corona}\]
Both boundary points are incoming, and the induced framing on these points are, left to right, respectively the last two points pictured in example~\ref{eg-framenot0}.
\end{example}

\begin{example}
Here is a picture of a $2$-framed manifold:
\[\textrm{filled in disc with corona}\]
The boundary is outgoing and is a circle with outward normal framing.
...
\[\textrm{four point cusped star with corona on top and bottom and arrow point out of the right two points; all points at 45 degrees} : \textrm{source} \ra \textrm{target}\]
...
\begin{tabular}{c|c}
point right fuzz & $\cC$ \\
point left fuzz & $\cC^\mp$ \\
line no arrows & $\bimod{\cC \btimes \cC^\mp}{\cC}{\Vec}$ \\
line arrows out & $\bimod{\Vec}{\cC}{\cC^\mp \btimes \cC}$ \\
circle with framing arrow & $\cC \btimes_{\cC \btimes \cC^\mp} \cC$
\end{tabular}
\end{table}
...
point [right, up] & $\cC$ &\\
point [left, up] & $\cC^\mp$ &\\
point [right, down] & $\cC^\op$ &\\
point [left, down] & $\cC^\mop$ &
\end{tabular}
...
\begin{table}[!h]
\begin{tabular}{c|cl}
right arc fuzz out & $\bimod{\cC \btimes \cC^\op}{\cC}{\Vec}$ 
& via $\cC^\op \xra{(-)^*} \cC^\mp$ \\
left arc fuzz out, arrows right & $\bimod{\Vec}{\cC}{\cC \btimes \cC^\op}$ 
& via $\cC^\op \xra{{}^*(-)} \cC^\mp$ \\
right arc fuzz in & $\bimod{\cC^\op \btimes \cC}{\cC}{\Vec}$ 
& via $\cC^\op \xra{{}^*(-)} \cC^\mp$ \\
left arc fuzz in, arrows right & $\bimod{\Vec}{\cC}{\cC^\op \btimes \cC}$ 
& via $\cC^\op \xra{(-)^*} \cC^\mp$ \\
