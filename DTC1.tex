...
\end{enumerate}
Both of these options may be used and give rise to equivalent ways to encode symmetric monoidal structures. The former option is only tenable if we have a tractable theory of weak functors at our disposal. For the case of symmetric monoidal categories we need a weak theory for 2-categories, where there is a well established and usable theory. This option is entirely reasonable in that case. 

However in this paper we ultimately wish to describe a symmetric monoidal 3-category. This generally requires a robust theory of 4-categories and their weak functors. It is possible to do this using the theory of iterated complete Segal spaces, which we will describe in the next section, but it just as easy (and perhaps more explicit) to pursue the later option from the start. 

In the case where the monoidal structure in question is defined via a universal property, (which is the case of our primary interest) this second option has a particularly tractable solution. Again, it is instructive to consider the 1-categorical case first. To this end we will now describe how the symmetric monoidal category of vector spaces may be obtained as a strict functor from $\Gamma^\text{op}$ to $\Cat$. 

\CSPcomm{[adding example of symetric mondoidal category. ]
\begin{itemize}
	\item tensor product as universal property
	\item $A$ goes to category of assignments of data. 
	\item strictness of the construction and segal conditions. 
\end{itemize}
}
