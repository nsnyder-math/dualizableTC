...
\subsection{Linear Categories, Tensor Categories, and Module Categories} \label{sec-tc-lincat}
...
\begin{definition}
	A {\em tensor category} is a linear monoidal category $(\cC, \otimes, 1, \alpha, \lambda, \rho)$ such that the functor $\otimes$ is multilinear. 
\end{definition}
...
\begin{remark}
	For a fixed object $x$ which admits a left dual, there is an obvious category whose objects $(x^*, \eta, \varepsilon)$ consist of those left duals, with choices of unit and counit maps. This category is easily shown to be {\em contractible} (equivalent to the terminal category). Hence the choice of a left dual is unique up to unique isomorphism. 
\end{remark}

\begin{definition}
	Let $\cC$ be a tensor category. A {\em left $\cC$-module category} is a linear category $\cM$ together with a multilinear functor $\otimes^{\cM}: \cC \times \cM \to \cM$ and natural isomorphisms
	\begin{align*}
		\alpha: & \;    [(-) \otimes (-)] \otimes^{\cM} (-)  \cong  (- ) \otimes^{\cM} [ (-) \otimes^{\cM} (-)] \\
		\lambda: & \; 1 \otimes^{\cM}(-) \cong id_{\cM},
	\end{align*}
	satisfying the evident pentagon and triangle identities. {\em Right $\cC$-module categories} are defined in an analogous manner. 
\end{definition}

\begin{example}
	Every linear category is a $\Vect_k$-module category in an essentially unique way. 
\end{example}
...
\CSPcomm{ (for later...)
\begin{itemize}
	\item A { \em bimodule functor} includes a morphism of tensor categories as part of the data. 
	\item An {\em $A$-$B$-bimodule functor} is one where that morphism is the identity.
	\item the category $\Fun_{\cC}(\cM, \cN)$ of (right exact) $\cC$-module functors and transformations is again a linear category. 
\end{itemize}
}

\subsection{Finite Linear Categories and the Deligne Tensor Product}
...
\begin{definition} % This is from EGNO Definition 1.18.2.
	...
	\begin{enumerate}
		...
		\item $\cC$ has enough projectives, i.e. every simple object of $C$ has a projective cover; and
...
\begin{definition}
	Let $\cA$ and $\cB$ be linear categories. The {\em Deligne tensor product $\cA \boxtimes \cB$} is the universal linear category admitting a multilinear functor $\boxtimes: \cA \times \cB \to \cA \boxtimes \cB$. That is, there exists a multilinear functor $\boxtimes: \cA \times \cB \to \cA \boxtimes \cB$ which induces, for all linear categories $\cC$, an equivalence between the categories of multilinear functors $\cA \times \cB \to \cC$ and linear functors $\cA \boxtimes \cB \to \cC$. 
\end{definition}
...
\begin{proposition}[Deligne (Prop 5.13), EGNO 1.46.2]
...
\begin{theorem}[EGNO 2.11.9(i)] \label{thm:EGNO2.11.9}
...
\begin{lemma}
	...
	\begin{equation*}
		\Fun_\cC(M, \cC) \boxtimes_\cC N \simeq \Fun_\cC(M,N).
	\end{equation*}
...
\subsection{Calcualtions for the Deligne Tensor product over finite tensor categories}
...
\begin{lemma}
If ${}_\cC\cM_\cD$ and ${}_\cC \cN_\cE$ are finite bimodule categories where the bimodule actions are left exact, then the map $\Fun_{\cC\text{-mod}}(\cM,\cC) \boxtimes_\cC \cN \rightarrow \Fun_{\cC\text{-mod}}(\cM,\cN)$ induced by $(f,n) \mapsto (m \mapsto f(m)n)$ is an equivalence of $\cD$--$\cE$ bimodules.
\end{lemma}
\begin{proof}
...
\begin{lemma}
If $\cM$ is a $\cC$--$\cD$ bimodule category and $\cN$ is a $\cC$--$\cE$ bimodule category, then taking left adjoints gives an equivalence of $\cD$--$\cE$ bimodules $\Fun_{\cC-\text{mod}}(\cM, \cN) \rightarrow \Fun_{\cC-\text{mod}}(\cN, \cM)^*$.  Similarly, if $\cM$ is a $\cC$--$\cD$ bimodule category and $\cN$ is a $\cD$--$\cE$ bimodule category, then taking left adjoints gives an equivalence of $\cC$--$\cE$ bimodules $\Fun_{\text{mod}-D}(\cM, \cN) \rightarrow {}^*\Fun_{\cC-\text{mod}}(\cN, \cM)$. 
\end{lemma}
\begin{proof}
We've already seen that the left adjoint has a canonical module functor structure.  To see that the actions match, note that taking adjoints interchanges precomposition and postcomposition, and the left adjoint of left multiplication by $x$ is left multiplication by $x^*$.  For the second statement, note that the left adjoint of right multiplication by $x$ is right multiplication by ${}^*x$.
\end{proof}
...
%the computations needed to identify the homotopy type of the structure spaces $BOrpo(3)$ and $BQuad$, and we discuss a few other, closely related structure groups.
...
\CSP{Do we need to mention stable structures?}
\begin{definition}
