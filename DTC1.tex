...
\begin{definition}
	...
	\begin{itemize}
		\item $F$ is the identity on objects,
		\item the restriction of $F$ to each hom category is linear. 
	\end{itemize}
...
If $\cM$ and $\cN$ are both $\cC$-$\cD$ bimodule categories, then bimodule functor which restricts to the identity functor on $\cC$ and $\cD$ will be called a {\em $\cC$-$\cD$-bimodule functor}.
