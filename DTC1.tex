...
Let $\cC, \cD \in \TCrig$, let $\cM, \cN \in \Hom_\TCrig(\cC,\cD)$, and let $\cF \in \Hom_{\TCrig}(\cM,\cN)$.  If the underlying functor of $\cF$ has a right (or left) adjoint as a functor, then $\cF$ has a right (resp. left) adjoint in $\Hom_{\TCrig}(\cN,\cM)$.
\end{lemma}
\begin{proof}
...
% No, don't need this other than it will come up in the proof of the first proposition below. \CDcomm{[Somewhere need to record the fact that $BSO(3) \xra{\widetilde{\phi \oplus \phi}} BSpin(6)$ is multiplication by $4$ on $\pi_4$.]}
...
& $0$ & $\ZZ/2$ & $0$ & $\ZZ/2$ 
& $\ZZ/2$ & $\ZZ/4$ & $0$ & $\ZZ/2$  \\
$Spin(6)/SO(3)$ % SO(3) --> Spin(6) --> Spin(6)/SO(3) in htpy; Spin(6)/SO(3) --> BSO(3) --> BSpin(6) in cohom
...
\begin{proposition}
There is a 4-connected map $Spin(6)/SO(3) \ra BOrpo(3)$ lifting the natural map $Spin(6)/SO(3) \ra BSO(3)$.  In particular, for a closed 3-manifold $M$, there is a bijection between the orpo structures and the 2-framings on $M$. 
\end{proposition}  % It seems the lift isn't unique.
\begin{proof}
... 
\CDcomm{In fact the choice of lift is canonical, using the null homotopy of $Spin(6)/SO(3) \ra BSO(3) \ra BSpin(6)$.  Indicate this.}
% Fill in this proof after finishing the computation of the cohomology of Spin(6)/SO(3).  Use email June 7.
\end{proof}
...
\begin{proposition}
There is a 4-connected map $O(\infty)/O(3) \ra BString(3)$ lifting the natural map $O(\infty)/O(3) \ra BO(3)$.  In particular, for a closed 3-manifold $M$, there is a bijection between the stable framings and the string structures on $M$.
\end{proposition} % It seems the lift isn't unique, though I haven't done the full calculation to be sure.
% We've omited mention of the fact that there is a natural forgetful map from stable framings to stable string structures, which happen to be equivalent to string structures, so in fact there is a preferred choice here.
\begin{proof}
...
\CDcomm{This map should also be canonical.  How best to say it homotopy theoretically?}
\end{proof}
