...
\subsection{Linear categories and tensor categories} \label{sec-tc-lincat}
	...
	\CSP{What is our notational difference for finite dimensional vector spaces versus all vector spaces.} \CD{I like the notation you picked.}
A {\em linear functor} is a right exact additive functor, which is also a functor of $\overline{\Vect}_k$-enriched categories. 
	\CSP{How do people feel about me putting right exactness into the definition of linear functor?} \CD{Can we add here a word about why, that might assuage readers who think it odd?}
...
\begin{proof}
Define the functor ${(-)^*}$ on objects by picking for each object $a \in \cC$ a left dual object ${a^*} \in \cC$.  Also pick a unit map $\eta: 1 \ra a \otimes {a^*}$ and a counit map  $\varepsilon: {a^*} \otimes a \ra 1$  giving ${a^*}$ the structure of a left dual to $a$.  Define the functor ${(-)^*}$ on a morphism $f: a \ra b$ to be its {\em mate}: \CD{In this proof, $a^*$ is used for the left dual? I'm guessing this just came from copying the text from my old proof while rewriting the formulas?}  \NS{I'm really confused by the notation here, why are there $\otimes$ some places and not others?}
\begin{equation*}
