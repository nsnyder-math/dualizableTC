...
\begin{lemma}
If $\cM$ is a finite $\cC$-$\cD$ bimodule category and $\cN$ is a finite $\cC-\cE$ bimodule category, then the map $\Fun_{\cC\text{-mod}}(\cM,\cC) \boxtimes_\cC \cN \rightarrow \Fun_{\cC\text{-mod}}(\cM,\cN)$ induced by $(f,n) \mapsto (m \mapsto f(m)n)$ is an equivalence of $\cD$--$\cE$ bimodules.
\end{lemma}
\begin{proof}
