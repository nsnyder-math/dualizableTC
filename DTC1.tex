...
\begin{definition}
A fusion category $\cC$ is \emph{pivotal} if it admits a {\em pivotal structure}, i.e. a monoidal equivalence between the double right dual functor $\ldd{(-)}: \cC \xra{\ld{(-)}} \cC^{\mop} \xra{\ld{(-)}} \cC$ and identity functor.
\end{definition}
...
\begin{theorem}[\cite{???}]
	\CSPcomm{A fusion category $\cC$ admits a canonical monoidal equivalence between the identity functor and the quadruple dual functor $\ldddd{(-)}$}.
\end{theorem}
...
\begin{itemize}
	\item This notion of spherical agrees with the standard one (left trace = right trace). NS, I think you had a proof of that? \CDcomm{I think it's in the Wave around June 10, in a post by Noah.}
...
\begin{itemize}
	...
	\item $\TCss$  finite semisimple tensor categories (i.e. multi-fusion), and finite semisimple bimodule categories. 
\end{itemize}
