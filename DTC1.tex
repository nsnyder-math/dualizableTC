...
\begin{example}
The set of 2-framings on a circle is a torsor for $\pi_1(SO(2))$.  We use our immersion notation to depict, as follows, a series of 2-framings on the standard circle that represent the elements of this torsor:
\[\cdots\quad
\cb{
\begin{tikzpicture}
{ [xshift=-3cm]
...
\nid Note that because a 2-framing induces an orientation, each of these circles may be identified with the standard circle up to orientation preserving diffeomorphism.
\NS{``The $2$-framings on the circle are naturally identified with $\mathbb{Z}$, and can be described by the following immersions:"}
\NS{Write the corresponding number below these.  Don't forget that 0 corresponds to the figure $8$.}
\CD{Sorry, Noah, I'm still not convinced that it is $\ZZ$ rather than a $\ZZ$-torsor.  It's isn't even a $\ZZ$-torsor until you pick a generator of $\pi_1(SO(2))$ but I presume we are doing that globally.}
\end{example}  
