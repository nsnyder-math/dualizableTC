...
\subsection{Notation for $n$-framed manifolds} \NS{I think this subsection should come earlier in the section} \CD{It's more or less independent, so could go anywhere.  Probably it will move to either just before or after the section on the CH.}
...
\subsubsection{Invariants of elementary 1-framed bordisms}
...
\end{lemma} \CD{I love the adjunction conventions used in this lemma, which probably means they are wrong ...}
\begin{proof}
Straightforward.
\end{proof}
...
\nid Notice that the first and fourth (and similarly the second and third) are equivalent, but not canonically equivalent, framed manifolds; their invariants are thus also equivalent, but not canonically so.  Of course, one could write down an equivalent TFT where all positive points are sent to $\cC$ and all negative points are sent to $\cC^{\mp}$, but this would break certain symmetries (in particular, you need to choose which way to identify the second and third points above, and there are two such natural choices).

A generating collection of 2-framed 1-manifolds have invariants as follows:
\begin{table}[!h]
\begin{tabular}{c|cl}
\cb{
\begin{tikzpicture}
...
\CD{I'm pretty sure all the circles on the list are (and were) distinct.}
...
These generators can be combined into various closed manifolds, such as the following:
\begin{table}[!h]
...
& $\cC \btimes_{\cC \btimes \cC^\mp} \cC$ & via $\cC^\mp \xra{(-)^{**}} \cC_2^\mp$ & \CDcomm{$\Fun_{\cC \btimes \cC^\mp}(\cC,\cC)$} \\
\cb{
\begin{tikzpicture}
\draw[linestyle,fuzzleft] (0,0) circle (\smcirclerad);
\end{tikzpicture}
}
& $\cC \btimes_{\cC^\mp \btimes \cC} \cC$ & via $\cC^\mp \xra{{}^{**}(-)} \cC_2^\mp$ & \\
\cb{
\begin{tikzpicture}
\draw[linestyle,fuzzleft,looseness=2]
(0,.5) to [out=0, in=10] (0,0)
...
& $\cC \btimes_{\cC \btimes \cC^\mp} \cC$ & via $\cC^\mp \xra{\id} \cC_2^\mp$ & \\
\cb{
\begin{tikzpicture}
\draw[linestyle,fuzzleft]
(.5,0) to [out=-90, in=-20] (0,-.4)
...
& $\cC \btimes_{\cC \btimes \cC^\mp} \cC$ & via $\cC^\mp \xra{(-)^{****}} \cC_2^\mp$ & \\
\cb{
\begin{tikzpicture}
\draw[linestyle,fuzzright]
(.5,0) to [out=-90, in=-20] (0,-.4)
...
& $\Vect \xra{k \mapsto \id} {\Fun_{\cC \boxtimes \cC^{\mp}}(\cC,\cC)}$ & \\
\cb{
\begin{tikzpicture}
\filldraw[linestyle,fill=\fillcolor] 
	(0,0) .. controls (.25,.25) and (.75,.25) .. (1,0)
...
We saw previously that every finite tensor category gave rise to a 2-dimensional 2-framed TFT.  This TFT is categorified, in the sense that diffeomorphisms of $2$-framed surfaces  with boundary give rise to automorphisms of the corresponding bimodule functors.  In fact, we can do better.  If $\cC$ is rigid there is a canonical lifting of this TFT to a $3$-framed $2$-dimensional TFT.  This fact is essentially equivalent to Radford's theorem for the quadruple dual of a finite tensor category.  This suggests that there should be a $3$-dimensional version of this TFT.  In fact, one can define the value of the TFT on certain non-invertible $3$-morphisms, getting some sort of ``non-compact" TFT.  We plan to address this in more detail in a later paper, so we will be brief in this section and not try for the strongest results.
...
The following argument can be thought of as a way to make precise the intuition that ``we have enough adjoints of $2$-morphisms to get a $3$-framed theory."

\begin{theorem}
Left Serre is canonically isomorphic to the right Serre with the iso given by the adjoints whose units have enough adjoints are biadjoints theorem.  \NS{clean this up}
\end{theorem}
...
We now define the universal right Serre automorphism, which is a particular $1$-morphism in the $n$-framed bordism $n$-category.  We presume $n \geq 2$ throughout.  For any point $p \in \RR^n$ we can equip the embedding $p \hra \RR^n$ with the canonical coframing, therefore n-framing.  We refer to such a point as the standard positively-framed point---it is an object of $\FrBord_0^n$, and we denote it by $s$.  Consider the normally-oriented immersed 1-manifold $\cS \lra \RR^2$ in Figure~\ref{fig-serre}.  This manifold is 2-framed, therefore n-framed for any $n \geq 2$.  It can be viewed as a morphism in $\FrBord_0^n$ from $s$ to $s$.  This automorphism is called the \emph{universal right Serre automorphism}.  
...
\end{tikzpicture}
\end{center}
\CD{This picture is the automorphism of the negatively oriented point.}

\begin{remark}
Consider the interval with one endpoint incoming, the other endpoint outgoing, and both endpoints being the standard positively framed point.  Framings on this interval form a group under composition and can be canonically identified with $\pi_1(\mathrm{SO}_n)$ (with the standard positively framed point as the basepoint).  When $n=2$, framings on the interval can be canonically identified with $\mathbb{Z}$.  The right universal Serre automorphism corresponds to $+1$ in this group, while the left Serre automorphism corresponds to $-1$.  When we refer to just the Serre automorphism, we mean the right Serre automorphism.
\end{remark}

The name is motivated by the following example.
...
Even better, there is a unique $3$-framed $2$-dimensional bordism of minimal genus from $\cS \circ \cS$ to the trivial, which we call the canonical trivialization of the square of the Serre.  (As usual, stabilizing this bordism gives a canonical trivialization of the square of the Serre for any $n > 2$.)  This bordism is depicted below as a $2$-manifold immersed in $\RR^3$ where the coorientation is out of the page and the bottom boundary is incoming while the top boundary is outgoing.

\begin{center}
\includegraphics{cobordism.pdf}
\end{center}

\NScomm{This diagram should be fixed up a bit.  First we need to indicate the incoming and outgoing boundaries, and second I think with our notation the left and right sides should be contracted down to a point.} \CD{We do not need to try to have the ``left and right" edges contracted to points in all the graphics.  It's conventional to allow squashing or expanding those edges to go between the two notations.}

\subsection{The Serre automorphism of $\TC$}
...
We now specialize to our case of interest, namely where the object in question is a finite tensor categroy (which is a dualizable object in the $(3,2)$-category $\TCrig$).  The answer to this calculation appears in the table in the previous section, but we make the calculation more explicit here.
...
\begin{lemma}
Suppose that $\cF$ is a $\cC$--$\cC$ bimodule equivalence $\cC(\id) \rightarrow \cC(\alpha)$, then $\cF(1)$ is invertible and $\alpha$ is naturally isomorphic as a monoidal functor to $\cF(1)^{-1} \cdot \cF(1)$.  Explicitly the natural transformation is given by $$c = \cF(1)^{-1} \cF(1) c \mapsto \cF(1)^{-1} \cF(c) \mapsto \cF(1)^{-1} \alpha(c) \cF(1),$$ where the first map is the natural transformation coming from left module functor structure on $\cF$ and the second map is the natural transformation coming from the right module structure on $\cF$.
\end{lemma}
\begin{proof}
$\cF(1)$ is invertible because is generates $\cC$ as a right module category.  The rest is clear.
\end{proof}
...
Recall that not only do we know that $\cC({}^{****}(-)) \cong \cC(\id)$, we actually have a canonical natural isomorphism given by the image of the canonical minimal genus surface described in the last section.  This gives us a canonical value for $X$, and gives a canonical natural isomorphism between the quadruple dual and conjugation by the canonical invertible object $X$.

\begin{theorem}
For a fusion category $\cC$ the canonical invertible is trivial.
\end{theorem}
\begin{proof}
