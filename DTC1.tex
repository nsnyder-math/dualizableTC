...
%\CSPcomm{How much background should we include on $n$-fold simplcial spaces? We need to make sense of the homotopy pull-back below. The version of complete n-fold Segal spaces that Jacob uses is constructed as a localization of the projective model structure on $n$-fold simplcial spaces. This is why we use the level-wise homotopy fiber product. Should these details be mentioned?}

\CSPcomm{[$n$-fold simplcial spaces, and $\Gamma$ objects in categories. How fiber products over a discrete base are homotopy equivalent to homotopy fiber products.]}

\begin{definition}
	An $n$-fold simplicial space $X$, regarded as a simplicial object $X_\bullet$ in $(n-1)$-fold simplicial spaces is an {\em $n$-fold Segal space} if the following conditions are satisfied
	\begin{enumerate}
		\item For every $m, k$, the diagram
		\begin{center}
		\begin{tikzpicture}
			\node (LT) at (0, 1.5) {$X_{m+k}$};
			\node (LB) at (0, 0) {$X_m$};
			\node (RT) at (2, 1.5) {$X_k$};
			\node (RB) at (2, 0) {$X_0$};
			\draw [->] (LT) -- node [left] {$$} (LB);
...
Now we describe the 3-category of tensor categories as a simplcial 2-category. 
\begin{itemize}
...
\begin{proof}
\CDcomm{[Should we include a proof?]}
\end{proof}
