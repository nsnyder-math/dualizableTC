...
To summarize, the 1-framed field theory $F_\cC$ associated to the finite tensor category $\cC$ takes the following values on the elementary 0- and 1-bordisms:\NS{For the lines don't you need to specify which direction the fuzz is going?} \CD{Not here, because these are immersed in $\RR^1$, so the normal bundle is zero-dimensional.}
\begin{table}[!h]
\begin{tabular}{c|c}
point right fuzz & $\cC$ \\
...
\nid The gray fuzz on the points here indicates the normal orientation of their immersions in $\RR^1$, and therefore specifies their 1-framing.  The small arrows indicate which boundary component are incoming (when the arrows point into the bulk manifold) and which are outgoing (when the arrows point out of the bulk manifold).
...
A representative collection of 2-framed 1-manifolds, along with their invariants, are given in the following table:
\CD{I am reading left to right and top to bottom.  If you want a bottom to top convention, scream now.}
\begin{table}[!h]
\begin{tabular}{c|c}
right arc fuzz out & $\bimod{\cC \btimes \cC^\mp}{\cC}{\Vec}$ \\
left arc fuzz out & $\bimod{\Vec}{\cC}{\cC \btimes \cC^\mp}$ \\
right arc fuzz in & $\bimod{\cC^\mp \btimes \cC}{\cC}{\Vec}$ \\
left arc fuzz in & $\bimod{\Vec}{\cC}{\cC^\mp \btimes \cC}$ \\
loop down fuzz up bordism & ... \\
loop up fuzz up bordism & ... \\
loop down fuzz down bordism & ... \\
loop down fuzz up bordism & ... \\
circle fuzz out & ... \\
circle fuzz in & ... \\
