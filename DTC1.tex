...
\begin{proof}
	\CSPcomm{ [To be filled in by NS or CSP]}
\end{proof}
...
\NS{This lemma might only be true with some exactness assumptions.  Otherwise the right adjoint might not be right exact, right?}
\begin{lemma}
...
\NS{I think this lemma is still fine though, because $\Fun_{\cC\text{mod}}(\cC,\cM)$ is $\cC$ regardless of whether $\Fun$ means right exact functors or left exact functors.}
