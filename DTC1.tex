...
In this section we review the theory of finite tensor categories and some related important definitions and results concerning module categories and the Deligne tensor product.  Most of this section is either standard or taken from the works of Etingof, Gelaki, Nikshych and Ostrik.  There are a few results in this section which are somewhat new (Theorem \ref{thm:DelignePrdtOverATCExists} and Lemma \ref{Lma:FunctorsAsATensorPdt}) as well as a several well-known results whose proofs do not appear in the literature. 
...
There are several different sorts of duality which appear in this work. These different notions of duality layer themselves together in a complex fashion. Moreover many of these dualities have been recognized previous, but brining them together has the potential for contradictory notation or terminology. The purpose of this section is to clarify our conventions.
...
\begin{definition}
	...
	\begin{align*}
		(id_{G} * \varepsilon  ) \circ (  \eta * id_{G}) &= id_{G} \\
		(\varepsilon * id_{F}) \circ (id_{F} * \eta) &= id_{F}.
	\end{align*}
We say $F$ is the {\em left adjoint} if $G$ and $G$ is the {\em right adjoint} of $F$. 
\end{definition}

Similarly we have:

\begin{definition} \label{def:duals}
	...
	\end{align*}
The object $x$ {\em admits a right dual} (denoted ${}^*x$) if it admits a left dual in $\cC^{mp}$. 
\end{definition}

\begin{remark}
	The potentially confusing notation $x^*$ for the {\em left} dual of $x$ is a product of the combination of two equivalent notions of duality, one coming from work on monoidal categories and the other coming from the notion of adjoint functor.  A convenient mnemonic is to think of the $*$ as eating things.
\end{remark}
...
The same observation applies to adjoint functors. 
\end{remark}

We extend this notation to bimodules as well.
\begin{definition}
	A bimodule ${}_A M_B$ {\em admits a left dual} if there is a bimodule ${}_B M^*_A$ (a {\em left dual}) and bimodule morphisms, the {\em unit} $\eta: {}_AA_A \to {}_AM \otimes_B M^*_A$ and the {\em counit} $\varepsilon: {}_BM^* \otimes_A M_B \to {}_B B_B$, satisfying the analogous pair of `Zig-Zag' equations. 
\end{definition}

This convention forces us to consider an $A$-$B$-bimodule as a morphism from $B$ to $A$. 
