...
\begin{theorem}
	Let $\cC$ be a finite rigid tensor category, and let $\cM$, $\cM_2$, $\cM_3$ be exact $\cC$-module categories, and $\cN$ an arbitrary $\cC$-module category. Then,
	\begin{enumerate}
		\item $\cM$ has enough projectives [EGNO lemma 2.7.1]
		\item In $\cM$ every projective object is injective, and vice versa. [EGNO Cor 2.7.4]
		\item Every functor $F \in \Fun_{\cC}(\cM, \cN)$ is exact. [EGNO prop 2.7.8]; Every additive enriched functor $F \in \Fun_{\cC}(\cM, \cM_2)$ is exact, and hence it has exact left and right adjoint.
		\item Composition $\Fun_{\cC}(\cM, \cM_2) \times \Fun_{\cC}(\cM_2, \cM_3) \to \Fuc_{\cC}(\cM, \cM_3)$ is exact in each variable. 
		\item $\Fun_{\cC}(\cM, \cM_2)$ is finite [EGNO Prop 2.13.5]; Hence $\Fun_{\cC}(\cM,\cM)$ is again a finite rigid tensor category. 
	\end{enumerate}
\end{theorem}
