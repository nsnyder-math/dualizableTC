...
\begin{example}
Fix an oriented circle.  A $2$-framing on that circle gives a map from that circle to the space of framed points.  Since the space of framed points is a canonically oriented circle, $2$-framings on the fixed circle are identified with $\mathbb{Z}$ via the winding number.  Changing the choice of oriented circle either preserves the identification of $2$-framings with $\mathbb{Z}$ or negates it, thus the identification of the space of framed points with $\mathbb{Z}$ is only natural up to sign.  Nonetheless, by choosing the fixed circle to be the unit circle with the standard orientation we can make a standard choice.  We use our immersion notation to depict, as follows, a series of circles corresponding to each element of $\mathbb{Z}$:\NS{I think this is stated rigorously now, but I think the diagrams currently are $3, 2, 1, 0, -1, -2, -3$}
\[\cdots\quad
\cb{
\begin{tikzpicture}
{ [xshift=-3cm]
...
& $\Vect \xra{1 \btimes 1} {\cC \btimes_{\cC \btimes \cC^\mp} \cC}$ & \\
\cb{
\begin{tikzpicture}
\filldraw[linestyle,fill=\fillcolor] 
	(0,0) .. controls (.25,.25) and (.75,.25) .. (1,0)
