...
There are several different kinds of duality which appear in this paper. These different notions of duality layer together in a complex fashion. Moreover many of these dualities have been recognized previously, and bringing them together has the potential for contradictory notation or terminology. The purpose of this section is to clarify our conventions.
...
\begin{definition}
	...
	\end{align*}
We say $F$ is the {\em left adjoint} of $G$ and $G$ is the {\em right adjoint} of $F$. \CD{Shall we use $\times$ instead of $\ast$ in those duality equations?}
\end{definition}
...
\begin{corollary}
	The following are equivalent for a finite semisimple tensor category $\cC$ (for example if $\cC$ is fusion):
	\begin{enumerate}
...
\NScomm{We should modify the above to reflect the fact that we only need exactness of tensor product, not rigidity} \CSPcomm{Unless I am mistaken, I don't think we even need that, just right exactness of the tensor product. When I wrote $\TCfin$ I really meant all finite tensor categories}
