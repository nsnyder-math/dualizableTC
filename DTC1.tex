...
It is well known that a finite linear category is equivalent to the category $\Mod{A}{}$ of finite dimensional modules over a finite dimensional $k$-algebra $A$. 

%\CD{[Fix the ground ring to be $\CC$?  Linear categories will mean linear over $\CC$?]}

\CSPcomm{... Deligne tensor product...}
...
Let $A$ be a finite set. We define a strict 2-category $\lincat(A)$ as follows. Let $P(A)$ denote the powerset of $A$. The objects of $\lincat(A)$ consist of  $P(A)$-tuples of objects on $\lincat$, that is a finite linear category $X(S)$ for each subset $S \subseteq A$, together with the following additional data. Whenever a subset of $A$ may be written as the disjoint union of two subsets $S$ and $S'$, we have a bilinear functor
\begin{equation*}
	x_{S,S'}: X(S) \times X(S') \to X(S \sqcup S')
\end{equation*}
which realizes $X(S \sqcup S')$ as the Deligne tensor product  $X(S) \boxtimes X(S') $. Whenever a subset of $A$ may be written as the disjoint union of three subsets $S$, $S'$, and $S''$, we have a natural isomorphism of trilinear functors 
\begin{equation*}
	\alpha_{S, S', S''}: x_{S \sqcup S', S''}\circ (x_{S,S'} \times 1) \cong x_{S, S' \sqcup S''} \circ (1 \times x_{S', S''}).
\end{equation*}
These natural isomorphisms satisfy the evident pentagon identity. Moreover we require that $X(\emptyset) \cong \Vect$, and that for each subset $S$, the bilinear functor $X(S) \times X(\emptyset) \to X(S)$ is the canonical one. 

A morphism $f:X \to Y$ in $\lincat(A)$ consists of a $P(A)$-tuple of linear functors $f_S: X(S) \to Y(S)$, together with natural isomorphisms of bilinear functors $\phi: y_{S,S'} \circ f_S \times f_{S'} \cong f_{S \sqcup S'} \circ x_{S,S'}$. These are required to satisfy the hexagon identity
\begin{equation*}
	\CSPcomm{add this...}
\end{equation*}

The 2-morphisms of $\lincat(A)$


 A functor $X:P(A) \to \lincat$ thus consists of an assignment of a finite linear category $X(S)$ for each subset $S \subseteq A$,  a linear functor $x_{S,S'}:X(S) \to X(S')$ for each inclusion $S \subset S'$, and coherent natural isomorphisms $x_{S', S''} \circ x_{S, S'} \cong x_{S,S''}$ for every pair of inclusions $S \subseteq S' \subseteq S''$. These are the objects of a strict 2-category $Fun(P(A), \lincat)$. Whenever a finite set can be expressed union of subsets of $A$ we obtain a bilinear functor
\begin{equation*}
\end{equation*}


Let $\lincat(A)$ denote the full sub-2-category of  $Fun(P(A), \lincat)$ consisting of those 


The Deligne tensor product on $\lincat$ induces a symmetric monoidal structure

 which is symmetric monoidal 

 

There exists a functor $\lincat:\Gamma^\op \to 2\Cat $, which is defined as follows. 
