...
\NScomm{This diagram should be fixed up a bit.  First we need to indicate the incoming and outgoing boundaries, and second I think with our notation the left and right sides should be contracted down to a point.}
...
\begin{definition}
Suppose that $\cC$ is a finite tensor category and $\cF: \cC \rightarrow \cC$ is a automorphism of tensor categories.  Let ${}_\cC \cC(\cF)_\cC$ denote $\cC$ as a $\cC$--$\cC$ bimodule where the action is $x \cdot c \cdot y = \cF(x) \otimes c \otimes y$.  \NS{Say something about the natural transformation}
\end{definition}
