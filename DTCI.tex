...
\begin{lemma}[{\cite[Prop 5.13(vi)]{deligne} }]\label{lem:Exact_in_each_is_exact}
	Let $\cM$, $\cN$, and $\cL$ be finite linear categories over a perfect field. Then a linear functor $\cM \boxtimes \cN \to \cL$ is exact if and only if the bilinear functor $\cM \times \cN \to \cL$ is exact in each both variables separately. 
\end{lemma} 
...
\begin{theorem}[\cite{EO-ftc, EGNO}] \label{Thm:ExactModCatOmnibus}
	...
	\begin{enumerate}
		...
		\CSP{(5) and (6) seem like they might depend on finiteness of M. Rearrange the order to make the clearer...}
	\end{enumerate}
\end{theorem}
...
\CSP{The proof of EO for item (5) is only valid if the unit is simple. (There are counter examples with multifusion categories). I need to add the general proof.}
