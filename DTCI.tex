...
\subsection{The Serre bordism and the Serre automorphism}
\CDh{\S 2.3 $\checkmark$}
...
%\CD{This section was missing information needed later.}
...
\begin{proof}[Proof of Proposition \label{prop-Cat_Radford}]
...
%\CD{This logic of this section was a complete mess. (and there were small mathematical issues)}
...
%\CD{This section had both a number of serious and also various minor mistakes and substantive ambiguities.}
...
%\CD{This section, including the main proof, had substantive issues and misdirections.}
...
%\CD{The formula before was wrong.}
...
\end{proof}
%\CD{The previous version of this proof had mistakes}
...
%\CD{The previous version of this proof was flat out wrong}
...
Over a perfect field (for instance, a finite field, an algebraically closed field, or a field of characteristic zero), an algebra $A$ is separable if and only if it is finite-dimensional semisimple; over an arbitrary field, separability is a stronger condition than finite-dimensional semisimplicity.  The corresponding statements hold for linear categories: over a perfect field, a finite $\Vect$-module category is separable if and only if it is finite semisimple, while over an arbitrary field separability merely implies finite semisimplicity. \CSP{References for classical separability} %!%CD
...
%\CD{Previous proof mixed obvious classical proofs and less obvious relevant material in an unnecessary way.}
...
%\CD{Previous proof very nice, has just been expanded to explain and highlight some of the subtleties.}
...
\subsubsection{The algebra of enriched endomorphisms of the unit}
...
\end{proof}
\CD{review use of alg closed and fusion in the above proof} % and the whole proof logic
\CD{is this where this theorem belongs?}


%No:{Define fusion to be finite tensor cat that is absolutely semisimple with unit absolutely simple.  (Then restrict to alg closed so obtain the usual notion.)}

%\CD{Various false statements fixed in this section.}
...
\end{proof}
%\CD{The proof here before was wrong.}
...
\end{proof}
%\CD{The proof here before was somewhere between unnecessarily complicated and wrong.  (Because it invoked two uses of the switch lemma.)}
...
\end{remark}
%\CD{The previous remark was problematic because semisimplicity hasn't been discussed}
...
% \CD{The statements here before had mistakes}
\begin{proposition} \label{prop:evcoev}
The pair of functors $(\eta_1,\varepsilon_1)$ form the unit and counit of an adjunction
\[
...
%\CD{Half the values in this table were wrong.}
...
\end{align*}
\end{theorem}
%\CD{The statement of this result was wrong in the previous version.}
\begin{proof}
...
as required.  
\end{proof}
%\CD{The paper was missing a proof of this main theorem.}
...
\end{table}
%\CD{The formulas here were also wrong.}
...
For our purposes, the most important 3-framed 2-manifold is the Radford bordism, depicted as an immersed surface in Figure~\ref{fig:Radford_bordism}.  Recall that this is the unique genus-zero 3-framed bordism from the left adjoint $\ev^L$ of the evaluation bordism to the right adjoint $\ev^R$ of the evaluation bordism.  As described in Section~\ref{sec:SerreRadford}, the inverse loop and loop bordisms are both obtained by composing the bordisms $\ev^L$ and $\ev^R$, respectively, with the evaluation bordism $\ev$.  As such, the Radford bordism may just as well be viewed as a bordism from the inverse loop bordism to the loop bordism.  
...
as desired.
\end{proof}
%\CD{The previous proof of this had numerous mistakes}
...
%\CD{What was here before was flat out wrong.}
...
\end{theorem}
\begin{proof}
By Proposition~\ref{prop:onedual}, a separable tensor category $\cC$ has $\cC^\mp$ as a dual tensor category; the tensor category $\cC^\mp$ is evidently also separable.  Combining Proposition~\ref{prop:evcoev} and Proposition~\ref{prop:dual-formula-for-adjoints}, a finite separable bimodule category ${}_\cC \cM_\cD$, between separable tensor categories, has the finite bimodule categories ${}^* \cM$ and $\cM^*$ as left and right adjoints.  By applying Corollary~\ref{cor:dualamod}, we see that the dual categories ${}^* \cM$ and $\cM^*$ are also separable.  Finally, Proposition~\ref{prop:bimodfunctadjoints} ensures that all bimodule functors between finite separable bimodule categories have adjoints.
\end{proof}
%\CD{The previous proof left out the only things that actually require checking.}
...
\begin{remark}
Though this corollary is a trivial consequence of the theorem, and the theorem is a straightforward application of the earlier constructions of duals and adjoints, this presentation conceals a substantial aspect of the work involved in proving that separable tensor categories are fully dualizable.  It is essential that the collection of tensor categories, bimodules, and functors that we can prove have duals and adjoints actually form a sub-3-category of the larger category $\TC$; that fact depends on Theorem~\ref{thm:compositeOfSep}, that separable bimodule categories compose.
\end{remark}
%!%\CD{I removed the remark on the semisimplicity of Z(C).  Could add back, with explanation and motivation.
...
%!% Check that char zero fusion => nonzero global dim.

% \CD{The stuff in this section seemed amiss previously, and also had mistakes.}
...
\end{proposition} %!% Rephrase in terms of fusion. 
\begin{proof}
...
% \CD{There were various mistakes in the above.}
