...
We will now fix a basis $\{e_{ij, k}^\alpha\}$ for each of the vector spaces $\Hom_{\cC}(L_i \otimes L_j, L_k)$ (where $\alpha$ runs over a finite indexing set which depends on $i,j$ and $k$). Since the above pairing is non-degenerate, we have an induced dual basis $\{\hat{e}_{ij, k}^\alpha \}$ of the vector space $\Hom_{\cC}(L_k, L_i \otimes L_j)$. Let $\{{}^*\hat{e}_{ij, k}^\alpha\}$ denote the induced basis of $\Hom_{\cC}({}^*L_j \otimes {}^*L_i, {}^*L_k)$. 

The multiplication map 
\begin{equation*}
	m: \oplus_{i,j} (L_i \otimes L_j) \boxtimes ({}^*L_j \otimes {}^*L_i) = A \otimes A \to A = \oplus_k L_k \boxtimes {}^*L_k
\end{equation*}
is given on summands by the sum $\sum_\alpha (e_{ij,k}^\alpha) \boxtimes ({}^*\hat{e}_{ij, k}^\alpha)$.


	
\CSPcomm{Evaluation and coevaluation maps as basis of a hom space. }
