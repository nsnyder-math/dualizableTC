...
\NS{remove this item}		\item If $\cM = \Mod{A}{}(\cC)$ and $\cN = \Mod{}{B}(\cC)$, for algebra objects $A \in \cC$ and $B \in \cC$, then $\cM \boxtimes_{\cC} \cN \simeq \Mod{A }{B}(\cC)$. Here $\Mod{A}{B}(\cC)$ denotes the category of $A$--$B$-bimodule objects in $\cC$.
		...
		\begin{equation*}
			\Hom_{\cM \boxtimes_{\cC} \cN} (m \boxtimes_\cC n, m' \boxtimes_\cC n') \cong \Hom_{\cC}(1, 
			 \IHom_{\cM}(m,m') \otimes \IHom_{\cN}(n, n')).
		\end{equation*}
		where $\IHom(-,-)$ denotes the $\cC$-valued enriched hom (see \cite{BTP} for details).
