...
\end{maintheorem}
\CSP{Should Thm1 and Thm 3 be combined?}
...
An $n$-dimensional topological field theory which takes values in an $(n+1)$-category is sometimes referred to as a `categorified' topological field theory.
\CSP{do we have a reference for `categorified'? or is it just word of mouth?}
 In dimension two such a field theory would sends surfaces to vector spaces rather than numbers and these vecotr spaces would inherit actions of mapping class groups. (A prototypical example of a categorified TFT is the concept of a {\em modular functor}).
...
When $\cC$ is a fusion category, the object $A$ is not only an algebra, but in fact is canonically a {\em Frobenius algebra} in $\cC \boxtimes \cC^\mp$. Recall that for Frobenius algebra object $R$ in a monoidal category we have the following maps:
\begin{equation*}
	...
	\end{array}
\end{equation*}
The pair $(u,m)$ give $R$ the structure of an algebra object, the pair $(\lambda, \Delta)$ give $R$ the structure of coalgebra object, and the pair $(b,c)$ give $R$ the structure of a self-dual object. The entire structure is completely determine by specifying the algebra structure $(u,m)$ and the counit $\lambda$.
...
We have already specified the algebra structure $(u,m)$ on $A$ as well as the counit $\lambda$. This gives us the pairing map $b= \lambda \circ m: A \otimes A \to 1 \boxtimes 1$. To show that this induces a Frobenius algebra structure on $A$ is suffices to construct a copairing (necessarily unique) $c: 1 \boxtimes 1 \to A \otimes A$ such that $(b,c)$ gives a self-duality for $A$. 	
...
\end{align*}
By construction we have $e_{i \overline{i}, 1} = \varepsilon_i$ and $\hat{e}_{i \overline{i}, 1} = \overline{\eta}_i$. It seems prudent to warn the reader at this point that, although it is true that $\overline{\overline{i}} = i$, in general we have $\overline{\eta}_i$ not equal to $\eta_{\overline{i}}$, and similar inequalities. 
...
The copairing $c: 1 \boxtimes 1 \to A \otimes A \cong \oplus_{i,j} (L_i \otimes L_j) \boxtimes ({}^*L_j \otimes {}^*L_i)$ has a equally explicit description. It is zero on all summands except those in which $L_j = L_{\overline{i}}$. On these latter it is given by
\begin{equation*}
...
\end{equation*} 
and see it is given by the global dimension of $\cC$. 
...
\end{equation*}
the other one is similar. In a moment we will expand the composite $(id_A \otimes b) (c \otimes id_A)$ in terms of the above formulas, but first let us make the following useful observation. Suppose that $x$ and $y$ are simple objects which happen to be isomorphic. Then there exists a canonical isomorphism
\begin{equation*}
	F: x \boxtimes {}^{*}x \stackrel{\cong}{\to}  y \boxtimes {}^*y 
\end{equation*}
in $\cC \boxtimes \cC^\mp$. Here $F = f \boxtimes {}^{*}(f^{-1})$ where $f: x \to y$ is any choice of isomorphism; the morphism $F$ is independent of the choice of $f$.

Thus we may expand the composite $(id_A \otimes b) (c \otimes id_A)$ in terms of the above formulas as follows (where $f_i: L_{\overline{\overline{i}}} \to L_i$ are any choices of isomorphism):
\begin{align*}
	&(id_A \otimes b) (c \otimes id_A) \\
	 &= \oplus_i [ (\varepsilon_i \otimes id_{L_{\overline{\overline{i}}}}) \boxtimes (id_{{}^* L_{\overline{\overline{i}}}} \otimes {}^* \overline{\eta}_i)] \circ [(id_{L_i} \otimes \overline{\eta}_{\overline{i}} ) \boxtimes ( {}^*\varepsilon_{\overline{i}} \otimes id_{{}^*L_i} ) \cdot d^2(L_i)] \circ  [f_i \boxtimes {}^*(f_i^{-1})] \\
	&= \oplus_i [(\varepsilon_i \otimes id_{L_{\overline{\overline{i}}}}) \circ (id_{L_i} \otimes \overline{\eta}_{\overline{i}} ) \circ f_i] \boxtimes [ (id_{{}^* L_{\overline{\overline{i}}}} \otimes {}^* \overline{\eta}_i) \circ ( {}^*\varepsilon_{\overline{i}} \otimes id_{{}^*L_i} ) \circ {}^*(f_i^{-1}) \cdot d^2(L_i)].
\end{align*}
These formulas are of course independent of the choices of $f_i$. 
...
\begin{align*}
	(\varepsilon_i \otimes id_{L_{\overline{\overline{i}}}}) \circ (id_{L_i} \otimes \overline{\eta}_{\overline{i}} ) \circ f_i \\
	(id_{{}^* L_{\overline{\overline{i}}}} \otimes {}^* \overline{\eta}_i) \circ ( {}^*\varepsilon_{\overline{i}} \otimes id_{{}^*L_i} ) \circ {}^*(f_i^{-1}) \cdot d^2(L_i)
\end{align*}
are the identity separately. 

We make two initial observations, first, the above formula is independent of the choice of $f$


Expanding the composite $(id_A \otimes b) (c \otimes id_A)$ in terms of the above formulas yields a morphism:
\begin{equation*}
	A = \oplus_i L_i \boxtimes {}^*L_i \to \oplus_i L_{\overline{\overline{i}}} \boxtimes {}^*L_{\overline{\overline{i}}} = A
\end{equation*} 
which in each $i$-component is given by:
\begin{equation*}
	 ...
\end{equation*}
(recall $d^2(L_i) = d^2(L_{\overline{i}})$).
...
%that this formula does indeed define the unique map making the pair $(b,c)$ into a self-duality of $A$.
 , but choosing $a$ carefully allows the formula for $c$ to be rewritten in terms of coevaluations and their duals.  
