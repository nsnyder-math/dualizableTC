...
Theorem~\ref{thm:EGNO2.11.6} also has the following important consequence. 

\begin{corollary}[\cite{BTP}] \label{cor:biexact_action}
	Let $\cC$ be a finite tensor category and let $\cM$ be a finite $\cC$-module category. Then the action map $\cC \times \cM \to \cM$ is automatically exact in each variable separately (i.e. for each $c \in \cC$ and $m \in \cM$ the functors $c \otimes (-)$ and $(-) \otimes m$ are exact).  
\end{corollary}
...
\begin{theorem}[\cite{BTP}] \label{thm:DelignePrdtOverATCExists}
	...
	\begin{enumerate}
		...
		%If the action maps $\cM \times \cC \ra \cM$ and $\cC \times \cN \ra \cN$ are both exact in each variable, and if $\cF: \cM \times \cN \to \cL$ is a $\cC$-balanced bilinear functor that is also exact in each variable, then the corresponding functor $\overline{\cF}: \cM \boxtimes_{\cC} \cN \to \cL$ is exact. 
	\end{enumerate} 
