...
\end{equation*}
Here $v_1$ is the bordism from Example~\ref{ex:saddle_bordism_immerssed} and $v_2^R$ denotes the right adjoint of the bordism depicted we previous called $v_2$.  
...
\NS{I moved this example from earlier in the section}
\begin{example}[The category of Hopf bimodules] \label{Ex:Hopf_bimod} \NS{I rewrote the references since the theorems have been moved out of this paper}
...
%The object $1 \in \cC$ is a $\cC$-projective $\cC$-generator (cf. example \ref{ex:rigid_all_C-proj}) and hence by Lemma \ref{lma:module-adjoint} and Theorem \ref{thm:C-module-Embedding} 
...
\NS{I think that's better left for the Radford section}
\end{example}
