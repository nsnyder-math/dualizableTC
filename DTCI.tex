...
\CD{Most of the acknowledgments now concern content in the other papers.  Perhaps they should shorten here / be less specific, and move to the others?}
\NS{Victor and Pavel are for this paper.  I'd suggest CD making the AH thanks less specific (the more specific one can go in the other papers).  The PT thanks may still be appropriate here, I'm not sure.}


\section{Local field theory in dimension three} \label{sec:lft}

\NScomm{We need to define what it means to say that a $3$-category is $1$-dualizable, $2$-dualizable, etc. so that the definitions are compatible with the statements in Section 4.}
...
\begin{definition}[Convention for adjoints] \label{def:Adjoints}
	The standard conventions for adjoint functors hold. That is, a functor $G: \cA \to \cB$  admits a left adjoint if there exists a functor $F: \cB \to \cA$ and natural transformations, the  unit $\eta: id_{\cB} \to G \circ F$ and the  counit $\varepsilon: F \circ G \to id_{\cA}$, satisfying the following pair of `zigzag' equations:
	\begin{align*}
...
\begin{definition}[Conventions for duals and bimodules].
\begin{itemize}
...
\item[2.] There is a bifunctor from $\Alg$ to $\Cat$ sending an algebra $A \in \Alg$ to the category $\Mod{}{A}$ of right modules.  Similarly there is a bifunctor from $\Alg$ to $\Cat^{op_1}$ taking $A$ to the category $\Mod{A}{}$ of left modules.  \CD{We should explicitly specify the kind of modules we'll mean we using the notation $\Mod{A}{}$, eg all, or finitely generated, ...}
\end{itemize}
...
%\CSP{The definition of tensor category should include rigid too, I think. Also finite. And the definition of finite should be moved here.}
\begin{definition}
...
%
Bimodule categories, functors, and transformations form a strict 2-category.  %!%CD check
...
\NS{In the long run, we might want to move this section and some other stuff into paper 0?  But we can initially post a long version to the arxiv, and then a shorter revision along with paper 0.}
...
%In this section we will temporarily relax our assumption that tensor categories are rigid. This will lead us to a clearer understanding of how this assumption is utilized in the remainder of this work.  \NS{Are you also relaxing "finite tensor category" to not mean rigid?  Maybe we should say "linear monoidal category" or "non-rigid tensor category."}
...
\end{theorem} \CD{Should this theorem be cited to Ostrik? ie move the reference from the previous line into the theorem line.}
...
\CSPcomm{\begin{itemize}
	\item Describe the bijection between descriptions (3) and (5) of the previous lemma.
	\item One of the maps is easy to describe. Its inverse involves choose isomorphisms $a:L_i \to {}^{**}L_i$, and (?) dividing each by the magnitude $d^2(L_i)$. (the fact that this is an isomorphism uses that the quantum trace of each factor is individually zero). 
	\item Now a simple calculation using this isomorphism given that $\mu \circ \Delta$ is multiplication by $\dim \cC$. 
	\item This is easiest to see in terms of string diagrams. 
\end{itemize}}	

\end{proof}


%\CSPcomm{We don't need the following anymore...}

%Let $z \in \cC$ be a simple object of a fusion category and let $y \in \cC$ be arbitrary. Then the pairing
%\begin{equation*}
%	\cC(y,z) \otimes \cC(z,y) \to \cC(z,z) \cong k
%\end{equation*}
%induced by composition is non-degenerate. Since these vector spaces are finite dimensional this induces a canonical copairing, an element
%\begin{equation*}
%	c \in \cC(y,z) \otimes \cC(z,y) \cong \cC(y,z)\otimes \cC({}^*y, {}^*z).
%\end{equation*}
