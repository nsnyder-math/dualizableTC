...
\subsection*{Higher categories} \CD{Other options for this title: ``What do we mean by a $3$-category?" or ``Higher category theory."}
\addtocontents{toc}{\SkipTocEntry}
...
$$\cdots \dashv v_{F,G}^{LL} \dashv v_{F,G}^L \dashv v_{F,G} \dashv v_{F,G}^R \dashv v_{F,G}^{RR} \dashv \cdots.$$  \NS{This sentence is very long and I think it should be chopped in two.}
...
\begin{example}\NS{Once the picture is done this example may need to be rephrased. --CSP: I changed it to match the picture. }
Figure~\ref{fig:belt_bordism} in the introduction depicts a $3$-framed $2$-manifold called the belt bordism.  Its source is the stabilization of the $2$-framed $1$-manifold 
\[
\cb{
\begin{tikzpicture}
...
\]
\CD{Replace the second tikz picture here. -- CSP: I fixed the tikz glitch, so I think it is fine now. }
...
\]
\CD{Painful tikz problems in this,  still unfinished. --CSP: I adjusted the "looseness" to 2, which seems to take care of it.}
...
When $n\geq 3$ we have $\pi_1(SO(n)) \cong \mathbb{Z}/2$, and therefore the loop bordism $\cS: pt_+ \ra pt_+$ in $\FrBord_n$ is an involution; thus the loop bordism and its inverse are isomorphic.  In fact, this isomorphism is implemented by the unique (up to $3$-framed diffeomorphism rel boundary) $3$-framed $2$-dimensional bordism of genus zero from $\cS^{-1}$ to $\cS$. By the above formulas for $\cS^{-1}$ and $\cS$, we see that giving a bordism from $\cS^{-1}$ to $\cS$ provides the same information as does giving a bordism from $\ev^L$ to $\ev^R$.  Again there is a unique such $3$-framed $2$-dimensional bordism of genus zero, which is depicted as an immersed $2$-manifold in $\RR^3$ in Figure~\ref{fig:Radford_bordism}. We call this bordism from $\ev^L$ to $\ev^R$ the {\bfseries Radford bordism} or belt bordism---if you wrap your belt into the configuration of the outgoing boundary and pull, it will happily (and very rapidly) trace out this bordism for you.
Figure~\ref{fig:Radford_bordism} also shows how this bordism is given explicitly as a composite
\begin{equation*}
	\cR = (v_1 \circledcirc id_{\ev^R})\circ (id_{\ev^L} \circledcirc v_2^R).
\end{equation*}
Here $v_1$ is the bordism from Example~\ref{ex:saddle_bordism_immerssed} and $v_2^R$ denotes the right adjoint of the bordism we previous called $v_2$.  
 

%This bordism is depicted, , in Figure~\ref{fig:loopinvloopbord}.
...
%\includegraphics[width=60mm]{cobordism.png}
%\end{center}
%\noindent We denote this bordism $\cB$ and call it the belt bordism As usual, stabilizing this bordism gives a canonical trivialization of the square of the loop bordism for any $n \geq 3$. 
...
If the dimension of the field theory $\cF$ is at least 3, then the image of the bordism in Figure~\ref{fig:Radford_bordism} is an isomorphism $\cR_x: \cF(\ev^L) \cong \cF(\ev^R)$, which we call the Radford isomorphism because, as we will see in section~\ref{sec:radiso}, it is a key ingredient in a topological proof of Radford's quadruple dual theorem. It also induces an isomorphism $\cS^{-1}_x \cong \cS_x$.  
...
\CDcomm{In reworking this section, add the calculation of $\cS_x$ and $\cS^{-1}_x$ with reference to App A.2.  Say, given a field theory, have these formulas; more is true, given a dualizable x, and any ev, evR, etc, can find a field theory such that these formulas hold---see App A.2 --- or maybe now it's the remark in section 4.1.  The formulas:
	\begin{align*}
...
Sections \ref{sec:conventions} and \ref{sec:tc-bimodules} are essential for all readers.  Section \ref{sec:tc-lincat} is background, we encourage experts to only refer back to it when needed, and encourage readers who are new to tensor categories to read \cite{BTP} for a more thorough explanation.  Sections \ref{sec:tc-separable} and \ref{sec:tc-fusion} are aimed at experts and  concentrate on the characteristic $p$ case.  \NS{I just added this paragraph} \CD{CD}
...
\begin{itemize}
	...
	\item[-] A linear category is {\em semisimple} if every object splits as a direct sum of simple objects, or equivalently if every short exact sequence splits. If in addition every object has finite length, then every object splits as a finite direct sum of simple objects. %!%CD
...
\CD{Move this lemma to where it is needed and/or provide some context/rationale for it being here.}
...
\begin{definition}
	A {\em linear monoidal category} is a monoidal category $(\cC, \otimes, 1, \alpha, \lambda, \rho)$ such that $\cC$ is a linear category and the functor $\otimes$ is bilinear. A {\em tensor category} is a rigid linear monoidal category.  \NS{I did a search through for ``tensor category" and the notational confusions of whether to say finite or rigid should be fixed now.  Tensor includes rigid but does not include finite.}
\end{definition}
\CD{Add (back) brief comment on why we'll stick to rigid?}
...
\end{example} \CD{tensor?}

The following lemma is crucial in constructing adjoints of $2$-morphisms in $\TC$.

\begin{lemma} \label{lma:module-adjoint}
Let $\cC$ and $\cD$ be finite tensor categories. Let  $\cM$ and  $\cN$  be finite $\cC$-$\cD$-bimodule categories and let $\cF: \cM \to \cN$ be a $\cC$-$\cD$-bimodule functor.  If the underlying functor of $\cF$ has a right (respectively left) adjoint as a functor, then $\cF$ has a right (respectively left) adjoint $\cC$-$\cD$-bimodule functor. 
\end{lemma} \CD{Why is this lemma here? Move to where it is needed?}
\CD{This is a comment in EO, left to the reader, search for "observe that an adjoint to a module functor has a natural structure of the module functor"}
...
\end{example} %!%CD
\CSP{I think we might want to add some material to this example discussing the alegbra $A = \IHom(1,1) \in \cC \boxtimes \cC^\mp$. CD: This comment was old and commented out --- is this still in play?}
...
\begin{theorem}[\cite{EO-ftc, EGNO}] \label{Thm:ExactModCatOmnibus}
	...
	\begin{enumerate}
		...
		\item $\Fun_{\cC}(\cM, \cM')$ is finite \cite[Prop 2.13.5]{EGNO}; in particular $\cM \simeq \Fun_{\cC}(\cC, \cM)$ is finite. %!%CDNC
...
\noindent (Recall that to say the family $\cP$ is {\em jointly generating} means the functor $\prod_\alpha \Hom_\cC(p_\alpha, -): \cC \to \Set$ is faithful.) \CD{Why defined this way?}
...
\begin{proof}  \NS{At some point me or CD should read this proof. CD: I not only read it, I rewrote chunks of it.}
...
\subsection{Dual bimodule categories} \label{sec:tc-bimodules} \NS{Is this the right title for this subsection?  I don't love it, but I also don't have a suggestion for a better one.}
...
\begin{lemma}
Suppose that $\cC$ and $\cD$ are finite tensor categories, and that $\cM$ is an exact $\cC$-module category, that $\cF: \cC \rightarrow \cC^\mop$ is an equivalence of tensor categories, and that $\cM^{\op}$ is the mod-$\cC$ given by $m \cdot c = \cF(c) \otimes m$.  Then $\cM^\op$ is an exact mod-$\cC$.
\end{lemma}
\begin{proof}
...
Suppose that $\cC$ and $\cD$ are finite tensor categories and that $\cM$ is an exact $\cC$--$\cD$ bimodule category, then $\cM^*$ and ${}^*\cM$ are exact $\cD$--$\cC$ bimodule categories.
\end{corollary}
...
We will need the following lemma which follows immediately from exactness of the tensor product for finite tensor categories \cite[2.1.8]{MR1797619} \cite[Prop. 1.13.1]{EGNO} \cite[Lemma 2.12.]{BTP}

\begin{lemma} \label{lem:sequence-adjunctions}  \NS{Lemma moved from BTP}
If $\cC$ is a finite tensor category, we have the following sequences of adjunctions of tensor product functors:
\begin{alignat*}{5}
...
\begin{lemma}
Suppose that $\cC$ and $\cD$ are finite tensor categories.  If ${}_{\cC}\cM_\cD$ is a $\cC$-$\cD$ bimodule category and ${}_{\cC}\cN_\cE$ is a $\cC$-$\cE$ bimodule category, then taking right adjoints gives an equivalence of $\cD$-$\cE$ bimodule categories 
...
We have already seen in Lemma \ref{lma:module-adjoint} that the adjoint of a module functor has a canonical module functor structure.  Furthermore, taking left adjoints is clearly an equivalence of the relevant functor categories; the inverse equivalence is given by taking right adjoints. Thus we need only check that the $\cD$-$\cE$ bimodule structures agree under this equivalence.  First note that taking adjoints reverses the order of composition. By Lemma \ref{lem:sequence-adjunctions} the right adjoint of right multiplication by $e$ is right multiplication by $e^*$. 
\end{proof}

\begin{lemma} \label{lem:dual-formula-for-adjoints}
Suppose that $\cC$ and $\cD$ are finite tensor categories. 
...
\NS{I agree with CD, so I commented out the old version and replaced it by one sentence referring forward to the remark.}
...
\end{warning}\NS{We refer to the fact in this warning later, so maybe it should be a lemma instead?}
...
\begin{lemma}
	...
	Similarly, we have an equivalences of $\cC$-module categories: 
	\begin{align*}
		{}^*(\Mod{}{A}(\cC)) & \simeq \Mod{A}{}(\cC) \\
		{}^*(L) & \mapsto {}^*L.
	\end{align*}
...
\end{theorem}
\begin{proof}
This follows from applying the previous theorem a bunch of times.  $\Vect$ separability of the bimodule categories is needed for the monoidal structure.  \NS{Maybe we should add more detail.}
\end{proof}
...
\CSP{Do we really need this paragraph? (regarding the center and $A$)}
...
\begin{definition}
Let $\cC$ be a fusion category and let $x \in \cC$ be an object. Given a morphism $a: {}^{**}x \to x$ the {\em quantum trace} $\Tr(a)$ of $a$ is defined as the composite: %\cite[pg 4]{MR2183279} % they cite BK pg 39.
\begin{equation*}
...
\begin{definition}
Suppose that $x$ and $y$ are simple objects satisfying $\dim \Hom(x \otimes y, 1) = 1$ in a fusion category $\cC$.  We define the following isomorphisms:
\begin{itemize}
...
%\end{equation*}
% This agrees with M\'uger's definition. This differs superficially from ENO's definition in that my x is their v**. So this would define d^2(v**) in their formulas, but since v and v** are isomorphic and this is invaraint under isomorphism this agrees with ENO's definition too. 
%where $a: {}^{**}x \to x$ is any choice of isomorphism and $(\varepsilon, \eta, \overline{\varepsilon}, \overline{\eta})$ is the corresonding ambidextrous duality.  This is easily seen to be independent of the choice of isomorphism between $x$ and its double dual (since $x$ is simple, different choices of $a$ only differ by multiplication by a non-zero scalar). Moreover the magnitude only depends on the isomorphism class of $x$. Also $d^2(x) = d^2(\overline{x})$ for any ambidextrous dual of $x$. 
%\begin{definition}
%	Let $\cC$ be a fusion category. The {\em global dimension} of $\cC$ is defined by the formula
%	\begin{equation*}
%		\dim(\cC) := \sum_{x} d^2(x)
...
\CSP{These should cite either ENO or BTP or both.}
\NS{I think the citations are now dealt with}
In order to apply the definition of separability, we need to find an explicit algebra $A$ in $\cC \boxtimes \cC^\mp$ with the property that $\cC \cong \text{mod-}A$ as a left $\cC \boxtimes \cC^\mp$-module category.  We use the following natural choice, which will also be used in comparing our Radford isomrophism to the one constructed in \cite{MR2097289} in \S \ref{sec:radiso}.

\begin{example}[The category of Hopf bimodules] \label{Ex:Hopf_bimod}  \NS{Moved and merged with the material already here}
	Let $\cC$ be a finite tensor category, and consider it as a left $\cC \boxtimes \cC^\mp$-module category. Let $A = \IHom(1,1)$ be the enriched endomorphism object of $1 \in \cC$; $A$ is an algebra object in $\cC \boxtimes \cC^\mp$.  Ostrik's internal hom $\IHom(1,1)$ is characterized by its functor of points; for every $x \in \cC$ and $y \in \cC^\mp$, we have a natural isomorphism:
\NS{I'm not sure when I'm supposed to say ``internal hom" versus ``enriched hom."  Someone else should fix anywhere I've used the wrong one.}
\begin{equation*}
...
\end{equation*}
for some object $c \in \cC$, unique up to isomorphism.  See \cite{MR1976459, EO-ftc, EGNO} and \cite[\S ~\ref{sec:conventions}--3.2]{BTP} for more detail on internal endomorphisms and this equivalence, and see \cite[\S 2]{MR2097289} for a detailed discussion of the category of Hopf bimodules.
\end{example}
...
%	There are several equivalent ways to describe a Frobenius algebra structure on a given algebra. We will use the following characterization: the counit map $\lambda: A \to 1 \boxtimes 1$ gives $A$ the structure of a Frobenius algebra if and only if the induced pairing
%	\begin{equation*}
...
\end{proof} %!%\CD{Not sure if that is sufficient. -- CSP: how is this?} 
...
\end{proposition} \CD{Maybe this should be a Proposition?}  \NS{Sounds reasonable, I changed it}
\begin{proof}
...
The other three calculations are similar.  \NS{I hope someone other than me has read this proof at some point.}
\end{proof}
...
\NS{CSP should we cite a particular result in your thesis?}
\end{proof}
...
\subsection{The Radford isomorphism of tensor categories} \label{sec:radiso} \NS{Someone other than me needs to edit this section.}
...
\vspace{.5in}
\nid \NScomm{repeat the usual image}
\vspace{.5in}
...
\subsection{The $SO(3)$ action and spherical structures}  \NS{Someone other than me needs to edit this section.}
...
\CD{A priori, f.d. objects are those in the essential image of the inclusion of $d\cC$ ...; model independence of notion of f.d. object ...}
