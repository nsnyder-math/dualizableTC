...
\CD{I vote for everything to be TFTs, not TQFTs}
...
\CSP{Do we want to cite BTP here?}
\NS{I don't think so, as the proof that relative Deligne tensor product is a category of functors is still in this paper.}
...
\CSP{do we have a reference for `categorified'? or is it just word of mouth?}
\NS{I don't know anywhere that this word is used.  Ben-Zvi--Nadler say ``two dimensional part of a three dimensional field theory."  I've tweeked the paragraph to remove any insuation that other people call it this.}
...
\subsection*{Model independence} \CD{I'm not actually sure what this subsection should be called.}
\NS{I vote for something more along the lines of ``What do we mean by a $3$-category?" or ``Higher category theory."  This paragraph isn't aimed at people who would understand what model independence is referring to.}
\addtocontents{toc}{\SkipTocEntry}
...
\begin{example}\NS{Once the picture is done this example may need to be rephrased.}
...
\CD{TD. Recall anything about nat transf of additive funcs between additive cats? cf 3.2.11}
...
\begin{theorem}[\cite{EO-ftc, EGNO}] \label{Thm:ExactModCatOmnibus}
	...
	\begin{enumerate}
		...
		\item Every additive (not a priori right exact) module functor $F:\cM \to \cN$ is exact \cite[Prop. 3.11]{EO-ftc} \cite[Prop 2.7.8]{EGNO}. In particular every functor $F \in \Fun_{\cC}(\cM, \cM')$ is exact, and has left and right adjoints (which are therefore themselves exact). Thus, $\Fun_{\cC}(\cM,\cM)$ is itself a finite tensor category. 
	\end{enumerate} %!% Middle sent CDNC cf Eg 3.2.6
\end{theorem}\CD{Discuss logic}
...
\begin{proof}  \NS{At some point me or CD should read this proof}
...
\begin{equation*}
	0 \to x'' \to x' \to x \to 0
\end{equation*}
where $x'$ is a finite direct sum of objects of the form $m \boxtimes n$. \CD{Why direct sum?}
...
\begin{itemize}
\item $\alpha_{x,y}: \Hom(x \otimes y, 1) \rightarrow \Hom(1, y \otimes x)$ characterized by $(f \otimes \id_x) \circ (\id_x \alpha_{x,y}(f)) = \id_x$.
...
\begin{equation*}
	b=\lambda \circ m: A \otimes A \to 1 \boxtimes 1
\end{equation*}
 is given by first projecting onto the factors where $L_j = L_{\overline{i}}$, and then applying $e_i \boxtimes {}^*\beta(e_i)$ to each factor separately. Thus there is only one possible copairing which by the definition of $\alpha$ is given on the $(L_{\overline{i}} \otimes L_i) \boxtimes ({}^*L_i \otimes {}^*L_{\overline{i}})$ component by $\alpha(e_{\overline{i}}) \boxtimes \alpha({}^*\beta(e_{\overline{i}}))$.
...
\end{remark}
\NS{This remark should be removed if we keep Appendix B}
...
\NS{Appendix A should be rewritten to be a gentle introduction.  We need to decide whether to keep any of the model independence here, or whether to move it all to 3TC.}
...
\NS{We need to decide whether to keep this appendix or move it entirely to 3TC}
