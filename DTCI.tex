...
\end{definition}
\CD{Add (back) brief comment on while we'll stick to rigid?}
...
\begin{example}[The category of Hopf bimodules] \label{Ex:Hopf_bimod} \NS{This example needs to be rewritten post BTP}
...
%{\em norm square} 
$\lVert x \rVert$ of $x$ is defined by the formula 
\begin{equation*}
...
It will be useful for us to have an alternate description of $\dim(\cC)$.  We first build some notation.

\begin{definition}
Suppose that $x$ and $y$ satisfy $\dim \Hom(x \otimes y, 1) = 1$.  We define the following maps:
\begin{itemize}
\item $\alpha_{x,y}: \Hom(x \otimes y, 1) \rightarrow \Hom(1, y \otimes x)$ via $(f \otimes \id_x) \circ (\id_x \alpha(f)) = \id_x$.
\item $\beta_{x,y}: \Hom(x \otimes y, 1) \rightarrow \Hom(1, x \otimes y)$ via $f \circ \beta(f)  = 1$.
...
In particular, we see that for nonzero $f: x \otimes y \rightarrow 1$ and $g: y \otimes x \rightarrow 1$ we have that $(f \circ \alpha(g)) \cdot (g \circ \alpha(f))$ is homogenous of degree $0$ in both $f$ and $g$ and thus gives an invariant not depending on the choice of non-zero $f$ and $g$.  It is easy to see that this invariant is exactly $\lVert x \rVert$.

Perhaps surprisingly the identifications $\alpha$ and $\beta$ fail to commute.
...
\end{lemma}
\begin{proof}
The first two are obvious.  For the third one, choosing $\alpha(g) = \beta(f)$ we have 
...
\begin{equation*}
	b=\lambda \circ m: A \otimes A \to 1 \boxtimes 1
\end{equation*}
 is given by first projecting onto the factors where $L_j = L_{\overline{i}}$, and then applying $e_i \boxtimes {}^*\beta(e_i)$ to each factor separately. Thus there is only one possible copairing which is given on the $(L_{\overline{i}} \otimes L_i) \boxtimes ({}^*L_i \otimes {}^*L_{\overline{i}})$ component by $\alpha(e_{\overline{i}}) \boxtimes \alpha({}^*\beta(e_{\overline{i}})$.
  
We now need to compute the composition of the pairing and copairing.  This is given on components by $(e_i \circ \alpha(e_{\overline{i}}) \cdot ({}^*\beta(e_i) \circ \alpha ({}^*\beta(e_{\overline{i}}))$.  Using the commutation relations, this is $\lVert x \rVert (e_i \circ \alpha(e_{\overline{i}})) \cdot ({}^*\beta(e_i) \circ {}^*\beta^{-1} (\alpha(e_{\overline{i}})$)).  Since this is homogeneous of degree $0$ in $\alpha(e_{\overline{i}})$ we may replace $\alpha(e_{\overline{i}})$ with $\beta(e_i)$.  Then the composition on the $e_i$ component is given by $\lVert x \rVert (e_i \circ \beta(e_i)) \cdot ({}^*\beta(e_i) \circ {}^*e_i) = \lVert x \rVert$.  The whole composition of pairing and copairing is thus given by the global dimension.
