...
Most previous examples of fully dualizable objects are in dimension $2$.  Ben-Zvi--Nadler \cite{???} prove $2$-dualizabilty for certain monoidal categories, but in the derived category setting instead of the abelian setting of this paper.  
\NS{What else should we mention here?  Chris's thesis? Bartels-Douglas-Henriques?  Freed-Teleman?}
...
A convenient mnemonic for this notation is to think of the $*$ as eating things.  

Although the above convention is shared throughout the literature, authorities are split on whether $x^*$ should be called the right dual of $x$ (following EGNO and BK)  or the left dual of $x$ (following Kassel, Chari-Pressley, Wikipedia).  Whichever choice is made we should have the additional meta-convention.

\begin{definition}[Convention C]
In reasonable settings where something could be called either a dual or an adjoint the left dual should be the left adjoint.
\end{definition}

For example, $\Fun(\cA, \cA)$ is a monoidal category where the duals are given by taking adjoints.  Also,any monoidal category can be thought of as a $2$-category with one ojbect, and in that setting the duals of objects in the monoidal category become adjoints of $1$-morphisms in the $2$-category.  However, in order for convention (C) to resolve the left/right ambiguity we need to decide whether $f \otimes g$ should mean do $f$ first or do $g$ first.

\begin{definition}[Convention for $\otimes$ and composition]
If $f$ and $g$ are functions then $f \otimes g$ will always mean do $f$ first. In other words, $f \otimes g = g \circ f$.
\end{definition}
...
The left dual functor is analogous.  Since the left and right duals are quasi-inverse to each other, they're each equivalences.
...
The following lemma is critical for proving our results.
...
A tensor category $\cC$ is a $\Vect$-separable if and only if it can be realized as a category of algebras over a separable algebra.  Thus $\Vect$-separability is well-understood.   $\Vect$-separability implies semisimplicity, and over a perfect field it is equivalent to semisimplicity.  More generally, each simple object $X$ in a semisimple category $\cC$ has a division ring of endomorphisms $D_X$ with center $K_X$, and $\cC$ is separable if and only if $K_X/k$ is a separable field extension.
...
\begin{theorem}
Separable finite tensor categories, finite $\Vec$-separable separable bimodule categories, bimodule functors, and bimodule natural transformations, form a symmetric monoidal $3$-subcategory of $\TC$.
\end{theorem}
\begin{proof}
This follows from applying the previous theorem a bunch of times.  $\Vec$ separability of the bimodule categories is needed for the monoidal structure
\end{proof}
...
\begin{theorem}
A $\Vect$-separable fusion category $\cC$ is seperable if and only if its global dimension is non-zero.
\end{theorem}
\begin{proof}
...
\begin{theorem}
If $\cC$ is separable and $\cM$ is an exact module category over $\cC$, then $\cM$ is separable.
\end{theorem}
\begin{proof}
...
In particular, over $\mathbb{C}$ we have that the separable finite tensor categories are exactly the semisimple finite tensor categories, and the separable bimodule categories are exactly the semisimple ones.
