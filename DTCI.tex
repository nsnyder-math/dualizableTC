...
where $a: {}^{**}x \to x$ is any choice of isomorphism and $(\varepsilon, \ets, \overline{\varepsilon}, \overline{\eta})$ is the corresonding ambidextrous duality.
...
Let us now make some observations about the algebra $A$. For each $x$ and $y$ have the following series of canonical isomorphisms:
\begin{align*}
...
\end{equation*}
We will see another explicite formula for the multiplication map momentarily. 
...
\begin{proof} To demonstrate the existence of this Frobenius algebra structure, it will be useful to employ another bit of structure available in fusion tensor categories. 
	Let $z \in \cC$ be a simple object of a fusion category and let $y \in \cC$ be arbitrary. Then the composition pairing
	\begin{equation*}
...
We will now fix a basis $\{e_{ij, k}^\alpha\}$ for each of the vector spaces $\Hom_{\cC}(L_i \otimes L_j, L_k)$ (where $\alpha$ runs over a finite indexing set which depends on $i,j$ and $k$). Since the above pairing is non-degenerate, we have an induced dual basis $\{\hat{e}_{ij, k}^a\}_a$ of the vector space $\Hom_{\cC}(L_k, L_i \otimes L_j)$. Let $\{{}^*\hat{e}_{ij, k}^a\}_a$ denote the induced basis of $\Hom_{\cC}({}^*L_j \otimes {}^*L_i, {}^*L_k)$. 

The multiplication map 
\begin{equation*}
	m: \oplus_{i,j} (L_i \otimes L_j) \boxtimes ({}^*L_j \otimes {}^*L_i) = A \otimes A \to A = \oplus_k L_k \boxtimes {}^*L_k
\end{equation*}
is given on summands by the sum $\sum_a (e_{ij,k}^a) \boxtimes ({}^*\hat{e}_{ij, k}^a)$.
	
	
	Since these vector spaces are finite-dimensional this induces a canonical copairing, an element
	\begin{equation*}
		c \in \cC(y,z) \otimes \cC(z,y) \cong \cC(y,z)\otimes \cC({}^*y, {}^*z).
	\end{equation*}
	\CSPcomm{dual basis... dual of evaluation/coevaluation. }

	\CSPcomm{ note that dual of evaluation and coevaluation do not satisfy the zig-zag identities (hence don't give a duality structure). Remark that we get a duality structure if we multiply one by the magnitude of the given object.}

	\CSPcomm{Identify the multiplication in terms of above copairing. }

	\CSPcomm{There is a unique normalized trace. This defines $b: A \otimes A \to 1 \boxtimes 1$. Write this out explicitly (uses evaluation and dual of coevaluation?)}

	\CSPcomm{ Forumla for the $c: 1 \boxtimes 1 \to A \otimes A$ (uses the coevaluation and the dual of the evaluation scaled by the magnitude.) Compute the window element as the global dimension.}
