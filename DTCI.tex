...
A pivotal tensor category $(\cC, P: (-) \simeq (-)^{**})$ is \emph{trace spherical} if for all objects $V \in \cC$, the quantum trace of the pivotal structure on $V$ agrees with the quantum trace of the pivotal structure on ${}^* V$, that is $\Tr(P_V) = \Tr(P_{({}^* V)})$.  \NS{Below we use the obviously equivalent $\Tr(P_V) = \Tr(P_{(V^*)})$.  Should we change this definition to match?}
\end{definition} 
