...
\CD{Add char p $Vect[\mathbb{Z}/p]$ (non ss composite) example?}
...
\begin{remark} \label{ref:components}
...
\begin{remark}
The assumption that $1 \in \cC$ is absolutely simple is unnecessary.  As in Remark \ref{ref:components}, $\cC$ breaks up into components $\cC_{ij}$ with $\cC_{ii}$ a fusion category.  It is not difficult to see that $D = \oplus D_i$ where $D_i$ is the distinguished invertible for $D_i$.  Thus, $D \cong \oplus D_i \cong \oplus \mathbf{1}_i \cong \mathbf{1}$.
\end{remark}
