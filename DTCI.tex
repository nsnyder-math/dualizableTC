...
In any finite tensor category, there is an invertible object $\Delta$ and a monoidal natural isomorphism between the quadruple dual functor $(-)^{****}: \cC \ra \cC$ and the conjugation functor $\Delta \otimes (-) \otimes \Delta^{-1}: \cC \ra \cC$.  In any semisimple finite tensor category with absolutely simple unit, the quadruple dual functor is monoidally naturally isomorphic to the identity.
\end{maincor}
...
Since we are working over a perfect field Lemma~\ref{lem:Exact_in_each_is_exact} applies, with the consequence that, for a general object $x \in \cM \boxtimes \cN$, it is enough to show that the functor
\begin{align*}
...
\begin{remark}
	The assumption that the ground field is perfect is necessary for the conclusion of Thm.~\ref{thm:tensor-exactness}. If $k$ is not a perfect field, then there exists a finite dimensional inseparable field extension $\ell$ over $k$. The category $\Vect(\ell)$ is semisimple, hence exact as a $\Vect(k)$-module category, however $\Vect(\ell) \boxtimes \Vect(\ell) = \mod{(\ell \otimes_k \ell)}{}$ is not. 
\end{remark}
...
\[
\Delta \otimes (-) \otimes \Delta^{-1} \simeq (-)^{****},
\] 
where $\Delta = D^{**}$.
\end{theorem}
