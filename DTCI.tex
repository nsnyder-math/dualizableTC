...
%It will be more convenient for us to use an alternative, equivalent formulation of dualizability focusing on ``$k$-dualizable $n$-categories" rather than ``$k$-dualizable objects."  A symmetric monoidal $n$-category $\cC$ is called ``$1$-dualizable" if every object has a dual---as before, here `dual' means dual in the symmetric monoidal $1$-category of objects of $\cC$ and equivalence classes of morphisms of $\cC$.  The symmetric monoidal $n$-category $\cC$ is called ``$2$-dualizable" if it is 1-dualizable and moreover every $1$-morphism has a left and a right adjoint---as before, here `adjoint' means adjoint in the $2$-category of objects, morphisms, and isomorphism classes of 2-morphisms of $\cC$.  The symmetric monoidal $n$-category $\cC$ is called ``$3$-dualizable" if it is $1$-dualizable and $2$-dualizable and if every $2$-morphism of $\cC$ has a left and a right adjoint.  (There are corresponding notions of $k$-dualizable objects of an $n$-category, for any $k$.)  An object of a symmetric monoidal $n$-category is $k$-dualizable if it is in some $k$-dualizable subcategory of $\cC$.  An $n$-dualizable object of a symmetric monoidal $n$-category is also called ``fully dualizable".
...
\begin{theorem}[\cite{BTP}] \label{thm:DelignePrdtOverATCExists}
	...
	\begin{enumerate}
		...
		%Let $F_0: M \to M'$ be a right $\cC$-module functor and let $F_1: N \to N'$ be a left $\cC$-module functor. These induce a $\cC$-balanced functor $F: M \times N \to M' \times N' \to M' \boxtimes_{\cC} N'$. If $F_0$ and $F_1$ are exact, then so is the induced functor $\overline{F}: M \boxtimes_{\cC} N \to M' \boxtimes_{\cC} N'$. 
		%If $\cF: \cM \times \cN \to \cL$ is a $\cC$-balanced bilinear functor that is also exact in each variable, then the corresponding functor $\overline{\cF}: \cM \boxtimes_{\cC} \cN \to \cL$ is exact. 
	\end{enumerate} 
	\CD{Confirm part (3), any conditions?}
	\CSP{I corrected part (5). There is now a proof of this in BTP.}	
	\CD{Discuss (5)} % Does it imply the non-relative old version?  If not, recheck 3.3.7.
\end{theorem} %!% Confirm happy with change.
\nid Tambara constructed an additive (rather than abelian) version of a relative Deligne tensor product for semisimple module categories over semisimple monoidal categories~\cite{tambara}.  Etingof--Nikshych--Ostrik constructed the relative Deligne tensor product for semisimple modular categories over a semisimple tensor category with simple unit over an algebraically closed field of characteristic zero~\cite{0909.3140}. \CD{Check that description of Tambara.}  \CD{Comb ENO for more refs}
...
\begin{lemma}[{\cite[Prop 5.13(vi)]{deligne}}]\label{lem:Exact_in_each_is_exact}
...
%As we work over not-necessarily algebraically closed fields, we need to strength their condition slightly in order to obtain the same conclusion.

\begin{definition}
Let $\cC$ be a finite tensor category.  A left $\cC$-module category $\cM$ is \emph{exact} 
%if the action functor $\cC \times \cM \ra \cM$ is exact in each variable, and 
...
%\CSP{Actually this parenthetical now seems to be wrong. See cor \ref{cor:biexact_action}}
...
\CSP{}
	\end{enumerate}
\end{theorem}
\nid \CSPcomm{\small I added the assumption of finite to the statement of this theorem. Certainly some results are valid without this, but it is easier to keep it. I changes the order (moving what was (5) to (3)). This now reflects the true logical dependance. I went through EGNO and have verified all that all of these arguments are correct and work over an arbitrary field (no perfect requirement). }
...
\CD{Check item 7 (... and the rest ...) really okay over arbitrary field}
\NS{Does this (eg 4, 5, 6) need perfect?}
\CSP{I think the above two comments have been addressed.}
...
\begin{proof}
	...
	\begin{tikzpicture}
			...
			\foreach \y in {2,4}{ % rows
				\foreach \x/\z in {1/2,2/3,3/4,4/5}{
					\draw [->, dashed] (m-\y-\x) -- node [above] {\text} (m-\y-\z);
				};		
			};
...
\CSP{This needs the ground field to be perfect: a counter example is given by tensoring the semisimple cat $L$-modules with itself, where L is an inseparable field extension. These are exact over $\Vec(k)$, since semisimple, but the tensor product is not.}
\end{theorem}
...
\CSP{This last part is redundant, now. }
\end{proof}
...
It is certainly not the case that every finite tensor category is 3-dualizable, but nevertheless the 2-dimensional field theory associated to a finite tensor category does always extend to take values on certain 3-manifolds and therefore provides a kind of ``non-compact" 3-dimensional field theory.  \CD{Add reference to in the spirit of Cos07 in dim 2 and generalization by Lur09 sec 4.2 here.}
...
\end{table}
\CD{Move this table to after first reference}
...
\end{corollary}
\CD{Add also a char 0 corollary}
