...
Although the above contention is shared throughout the literature, authorities are split on whether $x^*$ should be called the right dual of $x$ (following EGNO and BK)  or the left dual of $x$ (following Kassel, Chari-Pressley, Wikipedia).  Whichever choice is made we should have the additional meta-convention.
...
For example, $\Fun(\cA, \cA)$ is a monoidal category where the duals are given by taking adjoints, and any monoidal category can be thought of as a $2$-category and then the duals of objects in the monoidal category become adjoints of $1$-morphisms in the $2$-category.  However, in order for convention (C) to resolve the left/right ambiguity we need to decide whether $f \otimes g$ should mean do $f$ first or do $g$ first.
...
Throughout this section we consider only finite rigid tensor categories.  The goal of this section is to summarize Etingof and Ostrik's theory of exact module categories \cite{MR2119143}.
