...
Since rigidity is not preserved by Morita equivalence in $\TC$ (as pointed out to us by Dan Freed), there are finite tensor categories which are fully dualizable but not rigid.  We do not know a simple characterization of all fully dualizable tensor categories.  \CD{Should we speculate that the dualizable tensor categories are exactly the tensor categories that are Morita equivalent to fusion categories?}  \CD{Also point out that semisimplicity is not Morita invariant?}
...
\CDcomm{All of section 2 needs to be combed: remove material that is in TC3 but doesn't need to be here or can be shorter here.  Of course, much of the material may need to stay for review purposes.}
...
\end{definition} \CD{Maybe we no longer use twisted bimodule functors?}
